...
** TODO Plotting Voronoi
   ...
   #+BEGIN_SRC R :tangle voronoi.R :shebang #!/usr/bin/env Rscript
     ...
     vertex.path <-
       Map(function(node) vertices[[node]], path)
     vertex.path.x <- function(vertex.path)
       unlist(Map(function(vertex) vertex$x, vertex.path))
     vertex.path.y <- function(vertex.path)
       unlist(Map(function(vertex) vertex$y, vertex.path))
     ...
     qplot(tiles$x1,
       ...
       geom_path(aes(x=vertex.path.x(vertex.path),
                     y=vertex.path.y(vertex.path),
                     alpha=1.0),
                 color='red',
                 size=2.0) +
...
*** TODO Embedded R
    ...
    #+BEGIN_SRC c :tangle Reval.c
      ...
      void Reval(C_word c, C_word self, C_word k, C_word name, C_word args) {
        ...
        while (!C_truep(C_i_nullp(args))) {
          SEXP Rarg = R_NilValue;
          C_word arg = C_i_car(args);
          if (C_truep(C_i_flonump(arg))) {
            /* Setting a tag on the SEXP via SET_TAG simulates a named
               argument, incidentally; makes me wonder whether we can't do
               a little work in Scheme before calling Reval.
      
               We're going to have to formalize missing arguments for a
               first pass. */
            Rarg = ScalarReal(C_flonum_magnitude(arg));
          } else if (C_truep(C_fixnump(arg))) {
            Rarg = ScalarInteger(C_num_to_int(arg));
          } else if (C_truep(C_booleanp(arg))) {
            Rarg = ScalarLogical(C_truep(arg) ? 1 : 0);
          /* } else if (C_truep(C_numberp(arg))) { */
            /* Holding off on complex for the time being, since it's not
               supported in basic Chicken; have to load the numbers
               library. */
            /* Rcomplex complex = { */
          } else if (C_truep(C_stringp(arg))) {
            Rarg = ScalarString(Rf_mkChar(C_string_or_null(arg)));
          }
          SETCAR(iterexp, Rarg);
          args = C_i_cdr(args);
          iterexp = CDR(iterexp);
        ...
        printf("%d", error);
        printf("%d", TYPEOF(Rvalue));
        printf("%s", isList(Rvalue) ? "list" : "");
        printf("%s", isNewList(Rvalue) ? "newlist" : "");
        printf("%s", isPairList(Rvalue) ? "pairlist" : "");
        printf("%s", isVector(Rvalue) ? "vector" : "");
        printf("%s", isVectorList(Rvalue) ? "vectorlist" : "");
        if (!error) {
          /* Let's do something like
             <http://rcrowley.org/2008/03/03/functional-programming-in-c.html>
             in an array-pointing-to-functions, instead of this switch. */
          switch (TYPEOF(Rvalue)) {
          case REALSXP:
            {
              C_word *value;
              value = C_alloc(C_SIZEOF_FLONUM);
          ...
          case STRSXP:
            {
              char *string = CHAR(STRING_ELT(Rvalue, 0));
              C_word *value;
              value = C_alloc(C_SIZEOF_STRING(strlen(string)));
              C_kontinue(k, C_string(&value, strlen(string), string));
            }
          }
          /* return(value); */
        }
    ...
    #+BEGIN_SRC scheme :tangle R-test.scm
      ...
      #;
      (define (Reval function . args)
        #;($ scheme-object Reval (c-string function) (scheme-object args))
        ((foreign-primitive scheme-object
                            ((c-string function)
                             (scheme-object args))
                            "return(Reval(function, args));")
         function
         args))
      ...
      ;;; CPLXSXL
      ;; (Reval "NULL")
      ;;; Otherwise, package it up as an exotic; also: string evaluation.
