...
** TODO b
   ...
   #+BEGIN_SRC scheme
     ...
     ;;; Labels on edges: left, right, up, down; colors on nodes: white =>
     ;;; clean, gray => dirty, green => agent.
     (define (write-world-as-dot world agent)
       (display "digraph G {node [style=filled, fontname=monospace]; edge[fontname=monospace];")
       (hash-table-walk
        world
     ...
     (define (write-world-as-pdf world agent)
       (receive (input output id)
         (process "neato -Tpdf")
         (with-output-to-port output
           (lambda () (write-world-as-dot world agent)))
         (close-output-port output)
         (with-input-from-port input
           (lambda ()
             (do ((line (read-line) (read-line)))
         ...
         (hash-table-walk
          ...
          (lambda (from to)       
            (let* ((rest (make-list (- 4 (length to)) no-passage))
              ...
              (let ((left (list-ref to left))
                    (right (list-ref to right))
                    (up (list-ref to up))
                    (down (list-ref to down)))
                (if (passage? left)
                    (hash-table-update!/default
                     world
                     left
                     (lambda (left)
                       (debug (location-right left))
                       (if (no-passage? (location-right left))
                           (location-right-set! left from))
                       left)
                     (make-location (if (< (random-real) 0.5) clean dirty)
                                    no-passage
                                    from
                                    no-passage
                                    no-passage)))
                (if (passage? right)
                    (hash-table-update!/default
                     ...
                     right
                     (lambda (right)
                       (debug (location-left right))
                       (if (no-passage? (location-left right))
                           (location-left-set! right from))
                       right)
                     (make-location (if (< (random-real) 0.5) clean dirty)
                                    from
                                    no-passage
                ...
                (if (passage? up)
                    (hash-table-update!/default
                     ...
                     up
                     (lambda (up)
                       (debug (location-down up))
                       (if (no-passage? (location-down up))
                           (location-down-set! up from))
                       up)
                     (make-location (if (< (random-real) 0.5) clean dirty)
                                    no-passage
                                    no-passage
                ...
                (if (passage? down)
                    (hash-table-update!/default
                     ...
                     down
                     (lambda (down)
                       (debug (location-up down))
                       (if (no-passage? (location-up down))
                           (location-up-set! down from))
                       down)
                     (make-location (if (< (random-real) 0.5) clean dirty)
                                    no-passage
                                    no-passage
                                    ...
                                    no-passage)))
                (let ((location
                       (make-location (if (< (random-real) 0.5)
                                          clean
                                          dirty)
                                      left
                                      right
     ...
     (let* ((world (barabási-albert-graph))
            (start (random-node world))
        ...
        (compose-environments
         ...
         (lambda ()
           (let* ((location (agent-location agent))
             ...
             (case action
               ((left))
               ((right) (debug-print 'location (hash-table-ref world location)))
               ((up))
               ((down))
               ((noop))
