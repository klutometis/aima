...
* DONE 2.8
  ...
  #+BEGIN_SRC scheme
      ...
      (lambda ()
        (receive (agent score)
          ...
          (hash-table-update!
           agent->score
           agent
           (lambda (current-score)
             (update-score score current-score))
           (lambda ()
             #;
             (debug (agent-location agent)
                    (agent-actuate agent)
                    (agent-score agent)
                    (hash-table-keys agent->score)
                    (map (lambda (table-agent) (eq? table-agent agent))
                         (hash-table-keys agent->score))
                    (hash-table-ref/default agent->score agent 2)
                    (hash agent)
                    (hash-by-identity agent))
             (debug agent)
             initial-score)))))
    ...
    ;;; Do agent-composition here, too; for locationless agents. Most will
    ;;; have program (actuate) and scores, though.
    (define-record agent
      location
      ;; Program
      actuate
    ...
    (define make-vacuum-environment
      (case-lambda
       ((world agent)
        (make-vacuum-environment world agent agent->location))
       ((world agent agent->location)
        (lambda ()
          (let* ((location
                  (hash-table-ref agent->location
                                  agent
                                  (lambda ()
                                    (error
                                     (string-join
    ...
    (define (make-vacuum-performance-measure vacuum-world vacuum-agent)
      (lambda ()
        (let ((clean-squares
               (vector-count (lambda (i square) (clean? square)) vacuum-world)))
          (values vacuum-agent clean-squares))))
    
    (define make-vacuum-world vector)
    
    (let ((vacuum-world (make-vacuum-world dirty clean))
          (vacuum-agent (make-vacuum-agent right))
          (agent->score (make-hash-table eq? hash-by-identity)))
        ...
        (simulate
         (compose-environments
          ...
          (make-performance-measuring-environment
           ...
           0
           +
           ;; Does this make sense for multi-agents? We'd have to have one
           ;; performance-measure per agent. If the performance measure
           ;; were also responsible for updating, we could pull it off.
           (make-vacuum-performance-measure
            vacuum-world
            vacuum-agent))
          vacuum-environment))
        (map agent-score (hash-table-keys agent->score))))
  ...
  own score? Could we have an agent->score mapping in the environment
