...
* TODO 3.22
  ...
  #+BEGIN_SRC scheme :tangle search.scm
    ...
    (define (recursive-best-first start successors step-cost goal? heuristic)
      (let iter ((node (make-initial-node start))
                 (f-limit +inf))
        (if (goal? (node-state node))
            (predecessor-path node)
            (let ((successors (successors (node-state node))))))))
