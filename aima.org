...
* TODO 6.7
  ...
  (let ((domains (make-hash-table))
    ...
    (set-domains! domains
                  '(n1 n2 n3 n4 n5)
                  '(english spanish japanese ukrainian norwegian))
    (set-domains! domains
                  '(p1 p2 p3 p4 p5)
                  '(dog zebra snail fox horse))
    (set-domains! domains
    ...
    (set-pairwise-bidirectional-constraints!
     constraints
     '(n1 n1 n1 n2 n2 n3 n3 n4 n4)
     '(c3 c4 c5 c4 c5 c1 c5 c1 c2)
     (lambda (n c)
       (not (and (eq? n 'norwegian)
                 (eq? c 'blue))))
     (lambda (c n)
    ...
    (set-pairwise-bidirectional-constraints!
     ...
     '(d1 d2 d3 d4 d5)
     (lambda (s d)
       (not (and (eq? s 'snickers)
                 (neq? d 'orange-juice))))
     (lambda (d s)
       (not (and (eq? d 'orange-juice)
                 (neq? s 'snickers)))))
    (set-pairwise-bidirectional-constraints!
     constraints
     '(n1 n2 n3 n4 n5)
    ...
    (set-pairwise-bidirectional-constraints!
     ...
     '(s1 s2 s3 s4 s5)
     (lambda (n s)
       (not (and (eq? n 'japanese)
                 (neq? s 'milky-way))))
     (lambda (s n)
       (not (and (eq? s 'milky-way)
                 (neq? n 'japanese)))))
    (set-pairwise-bidirectional-constraints!
     constraints
     '(s1 s2 s2 s3 s3 s4 s4 s5)
    ...
    (let* ((neighbors (neighbors-from-constraints constraints))
      ...
      (debug (map hash-table->alist (backtracking-enumeration 1 csp)))))
  #+END_SRC
