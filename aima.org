...
** TODO Plotting Voronoi
   ...
   #+BEGIN_SRC R :tangle voronoi.R :shebang #!/usr/bin/env Rscript
     ...
     points <- unique(Reduce(function(points, row)
     ...
     for (p1 in points) {
       for (p2 in points) {
         debug(p1, p2, distance(p1, p2), furthest.points)
         if (!(is.null(p1) || is.null(p2)) &&
             distance(p1, p2) > furthest.points$distance)
           furthest.points <<- list(p1, p2, distance(p1, p2))
       }
     }
