...
** TODO Plotting Voronoi
   ...
   #+BEGIN_SRC R :tangle voronoi.R :shebang #!/usr/bin/env Rscript
     ...
     n.vertices <- 100
     vertices <- subset(deldir(rnorm(n.vertices), rnorm(n.vertices))$dirsgs,
                        !(bp1 | bp2))
     
     n.path <- 10
     path <- vertices[sample(nrow(vertices), n.path), ]
     
     start <- vertices[sample(nrow(vertices), 1), ]
     end <- vertices[sample(nrow(vertices), 1), ]
     
     points <- Reduce(function(points, row)
                      c(proto(x=row[[1]][[1]], y=row[[1]][[2]]),
                        proto(x=row[[1]][[3]], y=row[[1]][[4]]),
                        points),
                      init=NULL,
                      apply(vertices, 1, list))
     
     furthest.points <- list(p1=NULL, p2=NULL, distance=-Inf)
     
     distance <- function(p1, p2)
         sqrt((p1$x - p2$x)^2 + (p1$y - p2$y)^2)
     
     ## This thing is necessarily O(n^2), unless we can find a clever way
     ## to wrap the thing in a containing circle:
     ## <http://stackoverflow.com/a/6524446>.
     for (p1 in points) {
       for (p2 in points) {
         if (distance(p1, p2) > furthest.points$distance) {
           furthest.points$p1 <- p1
           furthest.points$p2 <- p2
           furthest.points$distance <- distance(p1, p2)
         }
       }
     }
     
     debug(furthest.points)
     ...
     ##   geom_point(aes(x=start$x1,
     ##                  y=start$y1,
     ##                  alpha=1.0),
     ##              shape=1,
     ##              size=5) +
     ##   geom_text(aes(x=start$x1,
     ##                 y=start$y1,
     ##                 label='Start',
     ##                 alpha=1.0),
     ##             size=3) +
     ##   geom_point(aes(x=end$x1,
     ##                  y=end$y1,
     ##                  alpha=1.0),
     ##              shape=1,
     ##              size=5) +
     ##   geom_text(aes(x=end$x1,
     ##                 y=end$y1,
     ##                 label='End',
     ##                 alpha=1.0),
     ##             size=3) +
     ##   geom_segment(aes(x=vertices$x1,
     ##                    xend=vertices$x2,
     ##                    yend=vertices$y2,
     ##                    yend=vertices$y2,
     ##                    alpha=1.0)) +
     ##   geom_text(aes(x=vertices$x1 + (vertices$x2 - vertices$x1) / 2,
     ##                 y=vertices$y1 + (vertices$y2 - vertices$y1) / 2,
     ##                 label=signif(sqrt((vertices$x2 - vertices$x1) ^ 2 +
     ##                   (vertices$y2 - vertices$y1) ^ 2),
     ##                   digits=2),
     ##                 alpha=1.0),
     ##             size=3) +
     ##   geom_path(aes(x=path$x1,
     ##                 y=path$y1,
     ##                 alpha=1.0),
     ##             size=2.0,
     ##             color='red') +
     ##   geom_point(aes(x=vertices$x1,
     ##                  y=vertices$y1,
     ##                  alpha=1.0))
