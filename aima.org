...
* TODO 4.12
  ...
  #+BEGIN_SRC scheme
    ...
    (define (make-agent-online-dfs start)
      (make-agent
       start
       0
               ...
               (begin
                 ...
                 (when previous-state
                   ...
                   (hash-table-update!
                    ...
                    previous-state
                    (lambda (state->action)
                      (hash-table-set! state->action points previous-action)
                      state->action)
                   ...
                   (hash-table-update!/default
                    ...
                    (lambda (backtracks)
                      ...
                      backtracks)
                    (make-stack)))
                 (if (stack-empty? (hash-table-ref/default untried points (make-stack)))
                     (if (stack-empty? (hash-table-ref/default unbacktracked points (make-stack)))
                         (begin
                           (debug 'not-found!)
                           (set! previous-action stop))
                          (let* ((backtrack (stack-pop! (hash-table-ref unbacktracked points)))
                                 (backtrack-action
                                  (hash-table-ref (hash-table-ref result points) backtrack)))
                            (debug 'harro)
                           ;; (debug backtrack
                           ;;        (hash-table->alist (hash-table-ref result points))
                           ;;        points)
                     ...
                     (begin
                       (debug 'warro)
                       (set! previous-action (stack-pop! (hash-table-ref untried points)))))
                 (set! previous-state points)
                 previous-action))))))
    ...
    (let ((world (tessellate 100)))
      (let ((points (tessellation-points world))
        ...
        (let* ((start (list-ref points (random (length points))))
               (agent (make-agent-online-dfs start)))
          (dotimes (step 10000 agent)
