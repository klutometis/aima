...
** TODO d
   ...
   #+BEGIN_SRC scheme :tangle tour.scm :shebang #!/usr/bin/env chicken-scheme
     (use aima-search
          debug
          define-record-and-printer
          heap
     ...
     (define (graph-tour tour strategy)
       (let ((data
              (hash-table-map
               tour
               (lambda (from to)
           ...
           (let ((x (R* ($ data x)))
             ...
             (R (plot (Reduce
                       ...
                       (list (geom_segment (aes x: x
                             ...
                             (ylab "y")
                             (labs color: "Strategy"
                                   title: (sprintf "Hamiltonian cycle of length %s"
                                                   (signif ,tour-length digits: 2))))
                       (ggplot))))
             (R (dev.off))))))
