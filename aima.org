...
*** TODO Depth-first graph constructor
    ...
    #+BEGIN_SRC scheme
      ...
      (define (make-unknown-node clean?)
         ...
         (let ((world (make-hash-table))
           ...
           (lambda (node clean?)
             ...
             (if (null? nodes)
                 ...
                 (if (not clean?)
                     'suck
                     (let* ((location (hash-table-ref/default world node (make-unknown-node clean?)))
                            ;; TODO: We have to munge the location with a
                            ;; previous node (if any) before calculating new-moves.
                            (new-moves (map direction->move (undiscovered-directions location))))
                       (debug location new-moves)
                       (if (null? new-moves)
                           (begin
                             (stack-pop! nodes)
                             (reverse-move (stack-pop! moves)))
                           (begin
                             (if (and (eq? (stack-peek nodes) node)
                                      (not (stack-empty? moves)))
                                 (let ((last-direction (move->direction (stack-pop! moves))))
                                   (debug `(vector-set! ,(location-neighbors location) ,last-direction ,no-passage))
                                   (vector-set! (location-neighbors location) last-direction no-passage))
                                 ;; This is redundant if this isn't a new
                                 ;; location; should do an update instead?
                                 (begin
                                   (debug `(hash-table-set! ,world ,node ,location))
                                   (hash-table-set! world node location)
                                   (if (not (stack-empty? moves))
                                       (let* ((last-node (stack-peek nodes))
                                              (last-location (hash-table-ref world last-node)))
                                         (let ((last-direction (move->direction (stack-peek moves))))
                                           (vector-set! (location-neighbors last-location)
                                                        last-direction
                                                        node)
                                           (vector-set! (location-neighbors location)
                                                        (reverse-direction last-direction)
                                                        last-node))))))
      ...
      (let* (
             ;; (world (make-preferential-depth-first-world 20))
