...
*** TODO Depth-first graph constructor
    ...
    #+BEGIN_SRC scheme
      ...
      (define (connect! world connectend connector direction)
        (let ((counter-direction
          ...
          (hash-table-update!/default
           ...
           connector
           (lambda (location)
             (vector-set! (location-neighbors location) counter-direction connectend)
             location)
           (make-dirty-location))))
      ...
      (write-world-as-pdf
       (make-preferential-depth-first-world 25)
       (make-reflex-agent 'harro (lambda (location clean?) 2))
       "world.pdf")
...
*** TODO Stateful graph agent
    ...
    #+BEGIN_SRC org
      ,- let
      ,  - world make-hash-table
      ,    # This is going to have to be a stack: we're iterating here, after
      ,    all. Ah, the problem is that we can't recurse back up the tree
      ,    because the relative directions are asymmetrical.
      
      ,    I knew this would bite me in the ass; or is there a more expensive
      ,    way to backtrack? Can't e.g. try random directions because there's
      ,    no way back.
      
      ,    Is it possible, somehow, to progress through the world via random
      ,    walk; turning away when we encounter a taken path? No, we need to
      ...
      ,      - location
      ,      - lambda location
      ,        - location-status-set! location (if clean? clean dirty)
      ,        - if eq? last-location location
      ,          - case last-action
      ,            - left location-set-left! my-location left no-passage
      ,            - right location-set-left! my-location right no-passage
      ,            - up location-set-left! my-location up no-passage
      ,            - down location-set-left! my-location down no-passage
      ,      - make-location unknown unknown unknown unknown unknown
