...
* TODO 3.22
  ...
  #+BEGIN_SRC scheme :tangle search.scm
    ...
    (define (uniform-cost start end path-cost children distance goal?)
      (best-first start end children path-cost distance goal? node-path-cost))
  ...
  #+END_SRC
    ...
    (define-record-and-printer tile
      moves)
        ...
        (let search ()
          (if (queue-empty? frontier)
              ...
              (let* ((parent (queue-remove! frontier))
                     (children (reachable-games k (node-state parent))))
                (debug (node-state parent))
                (hash-table-set! visited (node-state parent) #f)
                (debug (hash-table-size visited))
                (if (game= (node-state parent) end)
                    (predecessor-path parent)
                    (let ((unvisited-children
                           (filter (lambda (child)
                                     (hash-table-ref/default
                                      visited
    ...
    (let* ((k 2)
           (random-game (make-random-game k))
           (game (random-game)))
      (map node-state
           (search k
                   game
