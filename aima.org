...
* TODO 3.7
  #+BEGIN_SRC scheme :tangle 3.7.scm :shebang #!/usr/bin/env chicken-scheme
    (use aima-tessellation data-structures debug stack)
    
    (define (depth-first start end neighbors)
      (let ((visited (make-hash-table)))
        (let search ((frontier
                      (list (make-node start #f #f 0)))
                     (path '()))
          ;; (debug frontier)
          (if (null? frontier)
              (debug 'search-failed)
              (let* ((parent (car frontier))
                                    ...
                                    (make-node child
                                               ...
                                               (+ (node-path-cost parent)
                                                  (point-distance
                                                   ...
                                                   child))))
                                  unvisited-children)))
                        (search (append nodes (cdr frontier))
                                (cons parent path))))))))))
    
    (define (breadth-first start end neighbors)
      (let ((visited (make-hash-table))
            (frontier (make-queue)))
        (queue-add! frontier (make-node start #f #f 0))
        (let search ()
          (debug ;; (queue->list frontier)
                 (length (queue->list frontier)))
    ...
    (define (depth-first-general start end neighbors)
      (search start end neighbors make-stack stack-push! stack-pop! stack-empty?))
    
    (define (breadth-first-general start end neighbors)
      (search start end neighbors make-queue queue-add! queue-remove! queue-empty?))
    
    ;;; Generalized too quickly; each node should have: state, parent,
    ;;; action, path-cost.
    (let ((tessellation (tessellate)))
      (let ((start (tessellation-start tessellation))
        ...
        (let ((breadth-first (breadth-first start end neighbors))
              (breadth-first-general (breadth-first-general start end neighbors))
              (depth-first (depth-first start end neighbors))
              (depth-first-general (depth-first-general start end neighbors))
              )
          #;
          (debug (length depth-first)
                 ;; (length depth-first-general)
                 ;; (length breadth-first)
                 ;; (length breadth-first-general)
                 )
          (plot-tessellation
           tessellation
           depth-first
          ...
          (plot-tessellation
           tessellation
           breadth-first-general
           "Breadth-First-General"
           "breadth-first-general.png")
          (plot-tessellation
           tessellation
           depth-first-general
           "Depth-First-General"
           "depth-first-general.png")
          (plot-tessellation
           tessellation
           breadth-first
           "Breadth-First"
          ...
          ;; (plot-tessellation
          ;;  tessellation
          ;;  (list breadth-first depth-first)
          ;;  "Comparative"
          ;;  "comparative.png")
          )))
