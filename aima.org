...
** DONE d
   ...
   #+BEGIN_SRC scheme :tangle tsp.scm
     ...
     (define (graph-tour tour strategy output)
       (let ((data
              (hash-table-map
               tour
               (lambda (from to)
         ...
         (R (library "ggplot2"))
         (let* ((data (R (data.frame (t (matrix ,(list->vector data)
                                                nrow: 4)))))
                (data (R (colnames<- ,data (c "x" "y" "xend" "yend")))))
           (let ((x (R* ($ ,data x)))
                 (y (R* ($ ,data y)))
                 (xend (R* ($ ,data xend)))
                 (yend (R* ($ ,data yend))))
   ...
   #+BEGIN_SRC scheme :tangle 3.30.scm :shebang #!/usr/bin/env chicken-scheme
     ...
     (define (tour start graph points)
       (cons start
             (reverse
              (map node-state
                   (A* start
     ...
     (let* ((graph (complete-graph (make-random-points 10)))
            ...
            (start (car points)))
       (let* ((tour (tour start graph points))
              (tour (make-tour tour)))
         ;; (graph-tour tour)
...
* TODO 4.3
  ...
  #+BEGIN_SRC scheme :tangle 4.3.scm :shebang #!/usr/bin/env chicken-scheme
    ...
    (define (hill-climbing tour successor)
      (let iter ((tour tour))
    ...
    (let* ((points (list->vector (make-random-points 10)))
           (tour (shuffle points)))
      ;; (debug (tour-distance tour)
      ;;        (tour-distance (min-successor (successors tour))))
      (let ((tour (hill-climbing tour min-successor)))
