...
** DONE c
   ...
   #+BEGIN_SRC dot :file img/3-3c.pdf
     digraph G {
...
** DONE d
   ...
   friends.
   #+BEGIN_SRC dot :file img/3-3d.pdf
     digraph G {
       graph [size="0.5,0.5!", ratio=fill]
       edge [dir=none]
       a -> d -> e -> f
...
* TODO 3.4
  ...
  *symmetric group*, $S_7$, of $7!$ permutations.
  ...
  Pl_1 if and only if Cf2 is an even permutation Cf_1. Stated directly
  terms of the placements, we see that if Pl_1 and Pl_2 have the blank
  in the same cell then Pl_2 is obtainable from Pl_1 if and only if
  Pl_2 is an even permutation the 15 numberedblocks in Pl_1. Let $n$
  be the of number of moves the blank cell in Pl_1 is away from the
  blank cell in Pl_2. Since each move of the blank block causes a
  transposition two blocks, then for $n$ odd of (respectively even)
  Pl_2 is obtainable from Pl_1 if and only if Pl_2 is an odd
  ...
  #+END_QUOTE
* TODO 3.7
  See [[http://en.wikipedia.org/wiki/Convex_hull_algorithms][convex hulls]].
* Meetups
** Mon Jun 11 2012
