...
*** TODO Embedded R
    ...
    #+BEGIN_SRC c :tangle Reval.c
      ...
      C_word Reval(char* name, C_word args) {
        ...
        SEXP temp = CDR(expression);
      
        while (Rf_length(temp) > 0) {
          SEXP arg = NEW_INTEGER(1);
          INTEGER_DATA(arg)[0] = C_num_to_int(C_i_car(args));
          SETCAR(temp, arg);
          args = C_i_cdr(args);
          temp = CDR(temp);
        }
        ...
        int error = 0;
        SEXP value = R_tryEval(expression, R_GlobalEnv, &error);
        printf("%d", error);
