...
** TODO Plotting Voronoi
   ...
   #+BEGIN_SRC R :tangle voronoi.R :shebang #!/usr/bin/env Rscript
     ...
     leftest.point <- proto(x=Inf)
     rightest.point <- proto(x=-Inf)
     
     for (point in points) {
       if (point$x < leftest.point$x)
         leftest.point <<- point
     
       if (point$x > rightest.point$x)
         rightest.point <<- point
     }
     ...
     qplot(vertices$x1,
       ...
       geom_point(aes(x=leftest.point$x,
                      y=leftest.point$y,
                      alpha=1.0),
                  shape=1,
                  size=5) +
       geom_text(aes(x=leftest.point$x,
                     y=leftest.point$y,
                     label='Start',
                     alpha=1.0),
                 size=3) +
       geom_point(aes(x=rightest.point$x,
                      y=rightest.point$y,
                      alpha=1.0),
                  shape=1,
                  size=5) +
       geom_text(aes(x=rightest.point$x,
                     y=rightest.point$y,
                     label='End',
                     alpha=1.0),
                 size=3) +
