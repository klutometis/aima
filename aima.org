...
** DONE d
   ...
   #+BEGIN_SRC scheme :tangle tsp.scm
     ...
     ;;         (R (png ,output height: 900 width: 1600 type: "cairo"))
     ;;         (R (plot ,x ,y))
     ...
     (define with-R-output-to-png
       (case-lambda
        ((output thunk) (with-R-output-to-png output thunk 1024 768))
     ...
     (define plot-segments
       (case-lambda
        ...
        ((x y xend yend col)
         (R (segments ,x ,y ,xend ,yend col: ,col)))))
     ...
     (define (make-main main tour)
       (format "~a (length: ~,2f)" main (tour-length (make-tour tour))))
     ...
     (define (graph-tour-with-tree output main x-points y-points visited-points tree)
       (let-values (((x y xend yend) (points->vectors visited-points))
         ...
         (with-R-output-to-png
          output
          (lambda ()
            (plot-points x-points y-points (make-main main visited-points))
   ...
   #+BEGIN_SRC scheme :tangle 3.30.scm :shebang #!/usr/bin/env chicken-scheme
     ...
     (let* ((graph (complete-graph (make-random-points 30)))
            (points (hash-table-keys graph))
            (start (car points)))
       (let* ((tour (A*-tour start graph points "a-star.avi")))
         (graph-tour "a-star.png" "TSP with A* and MST" points tour)))
