...
** TODO b
   ...
   #+END_SRC
*** TODO Depth-first graph constructor
    ...
    #+BEGIN_SRC scheme
      ...
      (define (random-direction) (random 4))
      ...
      (define (random-start world)
        (let ((nodes (hash-table-keys world)))
          (list-ref nodes (random-integer (length nodes)))))
      ...
      (define (make-preferential-depth-first-world n-nodes)
                ...
                (let ((location
                  ...
                  (let ((n-neighbors (n-neighbors location)))
                    (if (and (< n-neighbors 4)
                             (< (random-real) (/ n-neighbors n-degrees)))
                        (let* ((new-directions
                                (vector-fold
      ...
      (define (make-barabási-albert-world n-nodes m)
        (let* ((world (make-seed-world))
               (start (random-start world)))
          (do ((current-n-nodes (count-nodes world) (count-nodes world)))
              ((= current-n-nodes n-nodes) world)
            (let ((new-node (make-node)))
              (let iter ((nodes (hash-table-keys world))
                         (m m))
                (if (and (pair? nodes)
                         (positive? m))
                    (let* ((node (car nodes))
                      ...
                      (if (and (< n-neighbors 4)
                               (< (random-real)
                                  (/ n-neighbors (count-degrees world))))
                          (begin
                            ;; (let ((connect?
                            ;;        (connect world node new-node)))
                            ;;   &c.)
                            (connect! world)
                            (iter (cdr nodes) (- m 1)))
                          (iter (cdr nodes) m)))))))))
      ...
      (define write-dot-preamble
        (case-lambda
         ((agent step)
          (write-dot-preamble agent step (default-width) (default-height)))
         ((agent step width height)
          (display "digraph G {")
          (display "node [style=filled, fontname=monospace];")
          (display "edge [fontname=monospace];")
          (if (and width height)
      ...
      (define write-world-as-dot
        (case-lambda
         ((world agent) (write-world-as-dot world agent #f))
         ((world agent step)
          (write-world-as-dot world agent step (default-width) (default-height)))
         ((world agent step width height)
          (write-dot-preamble agent step width height)
          (write-dot-nodes world agent)
          (write-dot-edges world)
          (write-dot-postscript))))
      ...
      (define write-world-as-gif
        (case-lambda
         ((world agent frame gif)
          (write-world-as-gif world agent frame gif (default-width) (default-height)))
         ((world agent frame gif width height)
          (receive (input output id)
            (process "neato" `("-Tgif" "-o" ,gif))
            (with-output-to-port output
              (lambda () (write-world-as-dot world agent frame width height)))
            (flush-output output)
            (close-output-port output)
            (close-input-port input)))))
      ...
      (define make-graph-animating-environment
        (case-lambda
         ((world agent directory)
          (make-graph-animating-environment world agent directory (default-width) (default-height)))
         ((world agent directory width height)
          (let ((frame 0))
            (lambda ()
              (let ((gif (make-pathname directory (number->string frame) "gif")))
                (write-world-as-gif world agent frame gif width height))
              (set! frame (+ frame 1)))))))
      ...
      (define (make-stateful-graph-agent start)
        (make-reflex-agent
         ...
         (let ((world (make-hash-table))
               ...
               (moves (make-stack)))
           (lambda (node clean?)
             (debug (hash-table->alist world) nodes moves)
             (if (stack-empty? nodes)
                 'noop
                 (if (not clean?)
                     'suck
                     (let ((location (hash-table-ref/default world node (make-unknown-location clean?))))
                                 ...
                                 (if (eq? (stack-peek nodes) node)
                                     ...
                                     (let* ((last-node (stack-peek nodes))
                                       ...
                                       (if (hash-table-exists? world node)
                                           ;; Cycle detected! Push the cycle-sentinel.
                                       ...
                                       ;; this node.
                                       (vector-set! (location-neighbors last-location)
                                                    (move->direction last-move)
                                                    node))))))
                       ;; Are there any other undiscovered passages?
                       (let ((new-moves (map direction->move (undiscovered-directions location))))
                             ...
                             (begin
                               ...
                               (if (stack-empty? moves)
                                   ...
                                   (let ((move (stack-pop! moves)))
                                     ...
                                     (reverse-move move)))) 
                             (let ((move (list-ref new-moves (random (length new-moves)))))
      ...
      (let* ((world (make-preferential-depth-first-world 20))
             (start (random-start world))
             ;; (world (make-known-world))
             ;; (start 'a)
             (agent (make-stateful-graph-agent start))
             ;; (agent (make-randomized-reflex-graph-agent start))
             )
        ;; (write-world-as-pdf world agent "graph.pdf")
        ;; (display-pdf "graph.pdf")
        (let ((directory (create-temporary-directory))
              (steps 50))
          (simulate
           ...
           (compose-environments
            ...
            ;; Can't this contain its finalizer? Maybe even give it the
            ;; terminal frame?
            (make-graph-animating-environment world agent directory 800 900)
            (make-finalizing-environment
             (make-animation-finalizer directory)
             steps)
            (make-debug-environment agent)
