...
* TODO 5.4
  We decided to implement Scrabble; see [[5.4-notes][notes to 5.4]].
...
* TODO 5.5
  ...
  TCP server where multiple clients connect; every client reads every
  message; relevant messages will be prefixed by a client’s ID. See
  [[http://stackoverflow.com/questions/10812920/tcp-server-with-multiple-clients-sending-message-back-to-all-connected-clients][this]].

  #+BEGIN_SRC scheme :comments link :tangle zmq-client.scm :shebang #!/usr/bin/env chicken-scheme
  (use debug posix zmq)
  ...
  #+BEGIN_SRC scheme :comments link :tangle zmq-server.scm :shebang #!/usr/bin/env chicken-scheme
  (use debug loops zmq)
  ...
  Since =req= and =rep= types work in [[http://api.zeromq.org/2-1:zmq-socket][round-robin]], it suffices for a
  client to connect: that secures its place in line (it can even come
  to the game late).

  The client receives a game state: board + rack; sends: start, word,
  orientation?

  #+BEGIN_SRC scheme :comments link :tangle scrabble-client.scm :shebang #!/usr/bin/env chicken-scheme
  ...
  #+BEGIN_SRC scheme :comments link :tangle scrabble-server.scm :shebang #!/usr/bin/env chicken-scheme
  (include "scrabble.scm")
  ...
  #+BEGIN_SRC scheme :comments link :tangle push.scm :shebang #!/usr/bin/env chicken-scheme
  (use debug posix zmq)
  (let ((socket (make-socket 'push)))
    (connect-socket socket "tcp://*:5555")
  (let iter ()
    (sleep 1)
    (debug (send-message socket "wassup"))
    (iter)))
  #+END_SRC

  #+BEGIN_SRC scheme :comments link :tangle pull.scm :shebang #!/usr/bin/env chicken-scheme
  (use debug zmq)
  (let ((socket (make-socket 'pull)))
    (bind-socket socket "tcp://*:5555")
    (let iter ()
  ...
  #+BEGIN_SRC scheme :comments link :tangle pub.scm :shebang #!/usr/bin/env chicken-scheme
  (use debug posix zmq)
  ...
  #+BEGIN_SRC scheme :comments link :tangle sub.scm :shebang #!/usr/bin/env chicken-scheme
  (use debug zmq)
  ...
  Hybrid server/pubsub model; endpoints: =new=, =join=, =list=,
  =play=.

  Advantage of one-file-per-endpoint model: an endpoint can go down,
  but not the whole app; advantage of the one-file-many-endpoint
  model: code reuse. The latter is a little more complex in the
  former: have to include a common file, &c.; also, some per-file
  boilerplate. Not to mention that the latter model has shared state.
  Going to have to persist to database otherwise.

  How do frameworks solve that problem: can the one endpoint 500 or is
  the whole app down?

  - =(new)= :: =gameid=
  - =(join gameid)= :: =userid=, =pubsub URL=
  - =(list)= :: =gameids=
  - =(play gameid userid move)= :: =status=
  - =(state gameid)= :: =state=

  Client can call state after they’ve subscribed to the pubsub stream
  to bootstrap themselves.

  Just give me a fucking key-value store; actually, I don’t want to
  even deal with serialization: let’s use a monolithic app and a
  hash-table.

  #+BEGIN_SRC scheme :comments link :tangle scrabble-service.scm :shebang #!/usr/bin/env chicken-scheme
  (use call-with-query
       define-record-and-printer
       hostinfo
       matchable
  ...
  #+BEGIN_SRC sh
    curl -v 'http://localhost/scm/aima/scrabble-service.scm/new' && \
        curl -v 'http://localhost/scm/aima/scrabble-service.scm/list' && \
        curl -v 'http://localhost/scm/aima/scrabble-service.scm/join?game-id=g0'
  #+END_SRC

* DONE 6.1
  CLOSED: [2013-06-30 Sun 16:37]
  There are 36 solutions for 3-color Australia: 12 for mainland and 3
  for Tasmania; there are 0 for 2-color Australia; 1152 for 4-color Australia.
  ...
  #+BEGIN_SRC scheme
    ...
    (define (make-k-color-csp-from-neighbors domain neighbors)
      (let ((variables (hash-table-keys neighbors))
            (constraints (hash-table-fold
                          neighbors
                          (lambda (whence whithers constraints)
                            (for-each
                                (lambda (whither)
                                  (hash-table-set! constraints (cons whence whither) neq?))
                              whithers)
                            constraints)
                          (make-hash-table))))
        (let ((domains (fold (lambda (variable domains)
                               (hash-table-set! domains variable domain)
                               domains)
                             (make-hash-table)
                             variables)))
          (make-csp domains constraints neighbors))))
    ...
    (define 3-colors (cons 'blue 2-colors))
    (define 4-colors (cons 'yellow 3-colors))
    
    (define 2-color-australia
      (make-k-color-csp-from-neighbors 2-colors australia))
    (define 3-color-australia
      (make-k-color-csp-from-neighbors 3-colors australia))
    (define 4-color-australia
      (make-k-color-csp-from-neighbors 4-colors australia))
    
    
    (test "3-color Australia count"
          36
          (length (backtracking-enumeration 3-color-australia)))
    
    (test "2-color Australia count"
          0
          (length (backtracking-enumeration 2-color-australia)))
    
    (test "4-color Australia count"
          1152
          (length (backtracking-enumeration 4-color-australia)))
    
  #+END_SRC
* DONE 6.2
  CLOSED: [2013-06-30 Sun 17:37]
  1. The variables are the placement of the knights.
  2. Possible values are pairs $([0 .. n - 1], [0 .. n - 1])$.
  3. Variables are constrained such that no other variable can be a
     knight’s jump ($(3, 2)$, $(2, 3)$ in any direction) away.
  4. There can be $\frak{1}{2}n^2$ knights (if $n$ is even),
     $\frak{1}{2}(n^2 + 1)$ if $n$ is odd. Place $k$ knights on the
     board; action: move knights; result: the knight is moved.
* DONE 6.3
  CLOSED: [2013-06-30 Sun 18:52]
  1. The search problem is effectively scrabble: see [[5.4]]; take
     best-first search with some heuristic, such as: length of word;
     or, to select for density, number of words touched by this word.

     Filling in the blanks one letter at a time can be efficient,
     given an appropriate data structure (e.g. [[http://en.wikipedia.org/wiki/GADDAG%0A][GADDAGs]]).
  2. As a constraint satisfaction problem, employ words; words are
     constrained such that they can only intersect with other words
     whose intersection is itself a valid word.

  The constraint-satisfaction problem might enable one to constrain
  the domain of possible words by forward propagation, such that one
  doesn’t have to reconsider the entire dictionary at every step.
* DONE 6.4
  CLOSED: [2013-06-30 Sun 19:10]
  1. In rectilinear floor planning, each variable is a small
     rectangle; value is: possible placements without overlapping
     other rectangles.
  2. In class scheduling, the assignments are tuples of the form
     $(professor, class, time, schedule)$; constraints: $(professor,
     class)$ must belong to set of enumerated $(professor, class)$
     values; no two assignments can overlap one another in time or
     class.
  3. In Hamiltonian tour, variables are the city visited at time $i$;
     such that no city $i$ can be a city visited at times $0 .. i - 1$
     or $i + 1 .. n$ and all cities must be visited.
* DONE 6.5
  CLOSED: [2013-06-30 Sun 19:37]
  Since none of the values is initially constrained, MRV doesn’t work;
  take the degree heuristic and select $O$. Try $O$ at 1; by forward
  checking, $R$ is 2 and $W$ is $\emptyset$.

  Backtrack to $O$; select 2. $R$ is 4 and $T$ is 1. Neither MRV nor
  the degree heuristic apply; take $W$ at random. $W$ can neither be
  $2$ nor $1$; choose $3$. $U$ is therefore $6$.

  Assign $5$ to $F$.
* DONE 6.6
  CLOSED: [2013-06-30 Sun 20:01]
  The ternary constraint $A + B = C$ can be turned into the binary
  constraints: $D = C$, $A$ is the first variable of the additive pair
  $D$, and $B$ is the second variable of the additive pair $D$.

  This recursive grouping can occur with $n$ variables even in
  non-commutative systems as long as order is preserved.

  Unary constraints on constituent variables can be propagated to
  composite variables by operating on the constiuent domains; in the
  example above, for instance, if $A$ is constained to be $0 .. 10$
  and $B$ is constrained to bo $10 .. 20$, $A + B$ is constrained to
  be $10 .. 30$.

  Unary constraints can be eliminated by preprocessing the domains of
  variables, pre-constraining them.
* TODO 6.7
  #+BEGIN_SRC scheme
    (use aima-csp
         debug
    ...
    (define zebra
      (make-csp
       (alist->hash-table '((n1 . (english norweigan))
                            (n2 . (english norweigan))
                            (h1 . (1 2))
                            (h2 . (1 2))))
       (alist->hash-table `(((n1 . h2) . ,(lambda (n h) (and (eq? n 'norweigan)
                                                        (= h 1))))
                            ((h2 . n1) . ,(lambda (h n) (and (eq? n 'norweigan)
                                                        (= h 1))))
                            ((h1 . h2) . ,(lambda (h1 h2) (= h1 (+ h2 1))))
                            ((h2 . h1) . ,(lambda (h2 h1) (= h2 (- h1 1))))))
       (alist->hash-table '((n1 h2)
                            (n2)
                            (h1 h2)
                            (h2 n1 h1)))))
    ...
    (define constraints (make-hash-table))
    ...
    (set-pairwise-constraints!
     ...
     '(h1 h2 h3 h4 h5)
     (lambda (x y)
       (xor (and (is? x 'kit-kat) (is? x 'yellow))
            (and (is? y 'kit-kat) (is? y 'yellow)))))
    
    ;; (debug (hash-table->alist (neighbors-from-constraints (csp-constraints zebra))))
    
    (define domains (make-hash-table))
    
    (set-domains! domains
                  '(h1 h2 h3 h4 h5)
                  '(english spanish norwegian japanese ukrainian
                            dog zebra snail fox horse
                            smarties snickers milky-way kit-kat hershey
                            red ivory yellow green blue
                            water coffee orange-juice milk tee))
    
    ;; (debug (hash-table->alist domains))
    
    ;; (debug (map hash-table->alist
    ;;             (backtracking-enumeration
    ;;              (make-csp domains constraints (neighbors-from-constraints constraints)))))
    
    ;; (debug (xor #t #f))
    
    (define (set-bidirectional-constraint! constraints x y constraint-x constraint-y)
      (hash-table-set! constraints (cons x y) constraint-x)
      (hash-table-set! constraints (cons y x) constraint-y))
    
    (let ((domains (make-hash-table))
          (constraints (make-hash-table)))
      (set-domains! domains '(n1 n2) '(E N))
      (set-domains! domains '(c1 c2) '(R G))
      ...
      (set-bidirectional-constraint!
       ...
       (lambda (c n)
         (and (eq? n 'E)
              (eq? c 'R))))
      (set-pairwise-constraints! constraints '(n1 n2) '(n1 n2) neq?)
      (set-pairwise-constraints! constraints '(c1 c2) '(c1 c2) neq?)
      (let* ((neighbors (neighbors-from-constraints constraints))
             (csp (make-csp domains constraints neighbors)))
        (debug (map hash-table->alist (backtracking-enumeration csp)))))
    
  #+END_SRC

  Ease of implementation vs. ease of specification is the difference
  between binary and n-ary CSPs:

  #+BEGIN_QUOTE
  We could transform any CSP into noe with only binary constraints;
  this makes the algorithms simpler. . . [On the other hand], it is
  easier and less error-prone to write the problem description using
  [e.g.] $Alldiff$.
  #+END_QUOTE

  Can we unleash pattern matching on this thing; is that cheating?
  That way, we could implement e.g. alldiff. Why don’t we ignore
  alldiff for the time being? It should be less stringent; and we
  should get redundant results; we’ll clean it up with ad-hoc global
  constraints.

  #+BEGIN_SRC scheme :comments link :tangle zebra.scm :shebang #!/usr/bin/env chicken-scheme
  (use debug
       matchable)
  ...
  Some macro-sugar, such as: =set-zebra-constraint!=.

  Results of running the binary constraints:
  ...
  For n-ary constraints, we’d probably have to do some ordering to
  make it feasible; unfortunately, we have to be aware of this
  ordering in the lambda. That’s messy; can we call it alphabetical?

  [[http://www.cs.mcgill.ca/~legri1/masters_thesis.pdf%0A][Here]]:

  #+BEGIN_QUOTE
  Inspired by Dalmau’s work on linear Datalog and Reingold’s
  breakthrough that undirected graph connectivity is in logarithmic
  space, we use a new restriction of Datalog called symmetric Datalog
  to identify a class of CSPs solvable in logarithmic space.

  The main result of this thesis is that directed st-connectivity and
  a closely related CSP cannot be defined in symmetric Datalog.
  Because undirected st-connectivity can be defined in symmetric
  Datalog, this result also sheds new light on the com- putational
  differences between the undirected and directed st-connectivity
  problems.
  #+END_QUOTE

  Can restrict ourselves to symmetric relationships? Also [[http://www.cis.temple.edu/~giorgio/cis587/readings/constraints.html][here]]:

  #+BEGIN_QUOTE
  A CS problem is usually represented as an undirected graph, called
  Constraint Graph where the nodes are the variables and the edges are
  the binary constraints.
  #+END_QUOTE

  The directed constraints don’t make sense in an n-ary context, do
  they? What does it mean to apply the constraint to e.g. =x, z, y=
  instead of =x, y, z=? YAGNI!

  That, or: different mechanism than hash-table for constraint
  lookups? It’s the hash-table that enforces order, I believe. What
  about the lambda? We need some well-defined order there, too.

  Or, instead of passing in values; we pass in some sort of
  variable-value combination; and we fish out the values we want in
  the order we want them?

  Can we do a hash-table lookup on =equal?=? (I feel like we’ve tried
  this before.) Nope:

  #+BEGIN_EXAMPLE
    #;111> (let ((table (make-hash-table #:test equal?))) (hash-table-set! table '(1 2) 'harro) (hash-table-ref table '(2 1)))
    ...
    #<hash-table (1)>
        ...
        <syntax>      (let ((table (make-hash-table test: equal?))) (hash-table-set! table (quote (1 2)) (quote harro)) (h......
        <syntax>      (##core#let ((table (make-hash-table test: equal?))) (hash-table-set! table (quote (1 2)) (quote har......
        <syntax>      (##core#begin (hash-table-set! table (quote (1 2)) (quote harro)) (hash-table-ref table (quote (2 1)...
        <syntax>      (hash-table-set! table (quote (1 2)) (quote harro))
        <syntax>      (quote (1 2))
        <syntax>      (##core#quote (1 2))
        <syntax>      (quote harro)
        <syntax>      (##core#quote harro)
        <syntax>      (hash-table-ref table (quote (2 1)))
        <syntax>      (quote (2 1))
        <syntax>      (##core#quote (2 1))
        <syntax>      (make-hash-table test: equal?)
        <eval>    (make-hash-table test: equal?)
        <eval>    (hash-table-set! table (quote (1 2)) (quote harro))
        <eval>    (hash-table-ref table (quote (2 1)))  <--
    #;111> 
  #+END_EXAMPLE

  We can enforce ordering during lookup, can’t we? Sure; just not in
  lambda: that’s too sticky. The constraint can be called with any
  order of variables.

  The key, therefore, for constraints is some ordered (proper) list of
  variables.

  With this model, though, we can’t reuse lambdas across scopes; we
  have to specifically look for variables =p1=, =p2=, etc. Is it
  ridiculous to name classes of variables? God, this is getting more
  and more ugly.

  Otherwise, resort to some convention that =relation-apply= will
  apply the values sorted alphabetically by variable.

  We can’t predict which order we’ll see the variables, either, given
  =select-unassigned-variable=; =consistent?=, on the other hand,
  always applies them in neighborly order. If we generate neighbors
  from constraints (which we’ll have to do with set-union now), we
  don’t have control over that order, however.

  Lot of work implementing n-ary constraints: not a quick surgery;
  punt.
  ...
  #+BEGIN_SRC scheme
    (define constraint (constraint-lambda (c d a b) (xor (and (a b) (c d)))))
    
    (constraint-set!/lambda constraints (c1 d1 a1 b1) constraint)
  #+END_SRC

  The idea is that the metavariables =(c d a b)= will be ordered the
  same as =(c1 d1 a1 b1)=. Should we check for that and error out?
  Maybe a warning.

  #+BEGIN_SRC scheme :comments link :tangle n-ary-zebra.scm :shebang #!/usr/bin/env chicken-scheme
  (use debug
       ;; expand-full
       list-utils
       symbol-utils)
  ...
  N-ary =alldiff= sucks, because it won’t be violated until all the
  variables are defined; let’s keep our pairwise alldiff. Also, if we
  keep a dag-representation of the graph, the
  =neighbors-from-constraints= thing will have to do the usual
  set-operations bidirectionally.

  Under n-ary, though, =alldiff= only has to apply to combinations,
  not permutations.

  Handle the case, too, where the scope to =constraint-lambda= is a
  single variable for e.g. =alldiff=; where the order and arity is
  irrelevant.

  When it comes time for application, how do we do lookups: based on
  the variables at hand, or some subset of them?

  Every combination, theoretically, right? Unary through n-ary; that’s
  where I wonder whether it wouldn’t be worthwhile to do a linear
  search on the constraints instead of generating every combination.

  In that sense, having a hash-table is nearly worthless; can we cache
  lookups on combinations of variables, somehow?

  If we have an ordering, this is where a tree-based representation
  would come in handy.

  Let’s do nested hash-tables and e.g. =every-applicable-constraint=;
  problem with this, though, is you have to distinguish between
  datum-leaves and datum-internal nodes. Nodes have a datum and a
  hash-table?

  Pass partial assignments to the constraints so that we can fail
  early?
* TODO 6.10
  #+BEGIN_SRC scheme :comments link :tangle random-coloring.scm :shebang #!/usr/bin/env chicken-scheme
    (use aima-tessellation
         aima-csp
         debug
         graphviz
    ...
    (define (intersect? a b c d)
      (and (xor (counter-clockwise? a c d) (counter-clockwise? b c d))
      ...
      (let iter-point ((points random-points)
        ...
        (if (null? points)
            (if modified?
                ...
                (debug (hash-table->alist connections)))
  ...
  See [[http://ktiml.mff.cuni.cz/~bartak/constraints/stochastic.html#MC][another explanation of min-conflicts]].

  Make some observations, average them out.

  Should we do the experiment on a process-by-process basis to avoid
  random garbage collection? Also, should stop any individual
  experiment after a certain amount of time; some maps are nearly
  insoluble, we should declare failure and move on.
* TODO 6.17
  See [[http://ktiml.mff.cuni.cz/~bartak/constraints/nosearch.html][min-cutset heuristics]]; [[http://www.me.utexas.edu/~bard/IP/Handouts/cycles%0A][cycles in undirected graphs]].

  Use partial information: order the nodes by cycles, do a
  depth-first-search to identify whether the node is on a cycle; cache
  the cycles for subsequent nodes? No: we have to identify whether
  there are still cycles after deleting; don’t bother.

  Partial info: depth first from highest node; cycles? No: continue
  depth first from all unvisited nodes; yes? Remove. Somehow to
  capture this notion of continuing to the next node.
  ...
  =Non-trivial-cycle?= that takes, e.g.: =graph=, =root=, =visited=;
  if =root= has been visited, no; otherwise, commence =dfs=, etc. Do
  this until cycle or no nodes left.

  Damn, could use a =for-each-enumeration=.
* TODO Logical agents
  The [[http://aima-python.googlecode.com/svn/trunk/logic.py][Python]] version is excellently documented: what, for instance,
  CNF entails; not to mention the [[http://www.cs.berkeley.edu/~russell/code/doc/overview-LOGIC.html][Lisp]] one.

  See [[http://w3.sista.arizona.edu/classes/ista450/spring13/projects/wumpus/][Arizona]]; see [[http://en.wikipedia.org/wiki/Satisfiability][satisfiability]]:

  #+BEGIN_QUOTE
  In mathematical logic, satisfiability and validity are elementary
  concepts of semantics. A formula is satisfiable if it is possible
  to find an interpretation (model) that makes the formula true. A
  formula is valid if all interpretations make the formula true. The
  opposites of these concepts are unsatisfiability and invalidity,
  that is, a formula is unsatisfiable if none of the interpretations
  make the formula true, and invalid if some such interpretation
  makes the formula false.
  #+END_QUOTE

  Let’s understand Norvig’s =build-truth-table=, therefore
  =compute-truth-entries=, =all-truth-interpretations=, &c.

  See this [[http://www-i2.informatik.rwth-aachen.de/i2/fileadmin/user_upload/pictures/Hybrid_Systems/Lectures/SATChecking/sat3a_propositional.pdf][high-level overview]]:

  #+BEGIN_QUOTE
  - Question: Is φ satisfiable? (Is ¬φ valid?)
  ...
  Let’s do [[http://en.wikipedia.org/wiki/DPLL_algorithm][DPLL]]? See this [[http://www.cs.rochester.edu/u/kautz/Courses/444au2010/program-sat-solver.html][assignment]]; also, [[http://en.wikipedia.org/wiki/Unit_propagation][unit propagation]]: based on
  CNF where clauses are disjunctions.

  [[http://en.wikipedia.org/wiki/Conjunctive_normal_form][CNF]] is a “conjuction of clauses, where a clause is a disjunction of
  literals.” [[http://en.wikipedia.org/wiki/DPLL_algorithm][DPLL]] is for CNF-SAT. “There exist transformations into
  CNF that avoid an exponential increase in size by preserving
  satisfiability rather than equivalence” (i.e. equisatisfiability).

  Can we have [[http://en.wikipedia.org/wiki/Negation_normal_form][elementary propositions]] which are composite: e.g. breeze
  at time 0 in square $(x, y)$?
  ...
  #+BEGIN_SRC scheme
    ...
    (define (->cnf formula)
      (match formula
        ;; ((? symbol? p) p)
        ;; De Morgan's over `and'
        (('not ('and . ps))
         (->cnf `(or ,@(map (lambda (p) (->cnf `(not ,(->cnf p)))) ps))))
        ;; De Morgan's over `or'.
        (('not ('or . ps))
         (->cnf `(and ,@(map (lambda (p) (->cnf `(not ,(->cnf p)))) ps))))
        ;; Eliminate the conditional.
        (('=> p q)
         (->cnf `(or ,(->cnf `(not ,(->cnf p)))
                     ,(->cnf q))))
        ;; Eliminate the biconditional.
        (('<=> p q)
         (->cnf `(and ,(->cnf `(or ,(->cnf p) ,(->cnf `(not ,(->cnf q)))))
                      ,(->cnf `(or ,(->cnf `(not ,(->cnf q))) ,(->cnf p))))))
        ;; Double negative
        (('not ('not p)) (->cnf p))
        ;; Distribution of conjuction over disjunction
        (('or p ('and . qs))
         (->cnf `(and ,@(map (lambda (q) (->cnf `(or ,(->cnf p) ,(->cnf q)))) qs))))
        ;; Literals
        (p p)))
    
    (debug (->cnf '(not (or p (=> q r))))
           (->cnf '(not (and p q)))
           (->cnf '(=> p q))
           (->cnf '(<=> p q))
           (->cnf '(or p (and q r s))))
  ...
  =Ands= that result from e.g. distribution or De Morgan’s should
  become additional clauses instead of =and=-clauses; that way, the
  representation is a list of literals (positive or negative).

  From Gomes:

  #+BEGIN_QUOTE
  When every clause of F has k literals, we refer to F as a k-CNF
  formula. The SAT problem restricted to 2-CNF formulas is solvable in
  polynomial time, while for 3-CNF formulas, it is already
  NP-complete.
  #+END_QUOTE

  Can we restrict ourselves to 2- or 3-CNF? Further Gomes:

  #+BEGIN_QUOTE
  For a partial assignment ρ for a CNF formula F, F|ρ denotes the
  simplified formula obtained by replacing the variables appearing in
  ρ with their specified values, removing all clauses with at least
  one TRUE literal, and deleting all occurrences of FALSE literals
  from the remaining clauses.
  #+END_QUOTE

  That sort of behavior depends or conjunction.

  Take a look at Norvig’s =->cnf=, by the way:
  ...
  He eliminates implications first, so =->cnf= can do its work; has
  =move-not-inwards= in its own function (and which handles e.g. =#t=
  and =#f=); has =merge-disjuncts=, which does some magic (e.g.
  becomes =#f= without arguments). He’s sensitive at every stage to
  returning e.g. =nil= when conjoining =#t=, =nil= when disjoining
  =#f=.

  =merge-disjuncts= is distributing conjunction over disjunction,
  isn’t it?

  This is an interesting case, and shows that our version isn’t quite
  sufficient:

  #+BEGIN_SRC lisp
    (->cnf '(AND (OR B (AND C (OR M N) F) D) (OR W(AND P (OR Q(AND X Y) X A)))))
    
    (AND (OR B C D) (OR B M N D) (OR B F D) (OR W P) (OR W Q X X A) (OR W Q Y X A))
  #+END_SRC

  This is the meaning of =merge-disjuncts=, etc.; our primitive
  pattern matching isn’t sufficient.

  #+BEGIN_SRC scheme :comments link :tangle satisfy.scm :shebang #!/usr/bin/env chicken-scheme
    (use debug
         matchable
         srfi-1
         test)
  ...
  For the time being, let’s assume the input is CNF; we’ll write a
  robust =->cnf= later.

  The input to DPLL is a single formula F; therefore, the world is
  just a conjunct of disjuncts.

  See [[http://www.mpi-inf.mpg.de/~sofronie/lecture-ar-09/slides/lecture-14-may.pdf][here]]:

  #+BEGIN_QUOTE
  - Unit clause ($N$ contains a clause $C$ such that all literals but
    one in $C$ are false under $A$)
  - Unit literal ($N$ contains a clause $C$ such that all literals but
    one in $C$ are false under $A$)
  - Pure literal (occurs only positively (or only negatively))
  #+END_QUOTE

  Can we therefore simply delete false literals? And can we delete
  clauses after assigning true? Yes (as we saw above):
  ...
  [[http://www.cs.unm.edu/~mccune/mace4/manual/2009-02A/syntax.html][This guy]] talks about the difference between terms, atomic formulas,
  literals, clauses, formulas. Formulas are more general than clauses;
  clauses seem to be a specific part of a CNF.

  We’re running into problems where we assume that a clause is a list;
  we have these naked clauses. Maybe that’s why they call disjuncts.

  Use a special object for a null-clause instead of the null-list?
  Makes debugging easier. Make some visualization instead?

  Need some unit tests for =simplify=.

  In retrospect, should have rendered all clauses a list of at least
  length one; this symbol exception is a pain-in-the-ass.

  We’re suffering from inconsistent representation, and should choose
  one over the other; =simplify=, for instance, receives =or’d=
  clauses but returns naked ones.

  Use the Russell-Norvigian “a entails b iff (and a (not b)) is
  unsatisfiable.”

  “The problem of determining the satisfiability of sentences in
  propositional logic-the SAT problem-was the first problem proved to
  be NP-complete. Many problems in computer science are really
  satisfiability problems. For example, all the constraint
  satisfaction problems in Chapter 6 ask whether the constraints are
  satisfiable by some assignment.”

  I suspected this to be the case; should have done the problems in
  the back.

  Things like the state of the wumpus as state-variables? Let’s fold
  them into the knowledge base. (Time variables as state variables: a
  little cheating.)

  Initial simplification: teleportation.

  Some operation for the merging of KBs.

  Successor-state axioms

  Make $n$ and $m$ parameters. Do we have to explicate the
  not-actions? I hope not.

  Wow, if the database becomes inconsistent: you can ask it anything
  you want!
* TODO Random agent against wumpusworldserver.com
  #+BEGIN_SRC scheme
    (use alist-lib
         debug
         define-record-and-printer
    ...
    (define token (make-parameter (get-environment-variable "TOKEN")))
    (define board-id (make-parameter (get-environment-variable "BOARD_ID")))
    (define game-id (make-parameter (get-environment-variable "GAME_ID")))
    
    (define base-url (make-parameter (uri-reference "http://wumpusworldserver.com")))
    
    (define (add-token uri) (update-uri uri query: `((token . ,(token)))))
    
    (define (new-board-url)
      (update-uri (base-url) path: '("" "api" "board" "new" "5" "5")))
    
    (define (new-game-url board-id)
      (update-uri (base-url) path: (list "" "api" "board" board-id "game" "new")))
    
    (define (action-url action)
      (update-uri (base-url) path: (list "" "api" "board" (board-id) "game" (game-id) action)))
    
    (define (status-url) (action-url "status"))
    (define (forward-url) (action-url "forward"))
    (define (left-url) (action-url "left"))
    (define (right-url) (action-url "right"))
    (define (shoot-url) (action-url "shoot"))
    (define (grab-url) (action-url "grab"))
    ...
    (define (read-url-json url)
      (read-url url read-json))
    
    (define-record-and-printer maybe)
    (define maybe (make-maybe))
    
    (define-record-and-printer state breeze stench pit wumpus)
    
    (define (update-world! world x y state-sense-set! state-thing-set!)
      (hash-table-update!/default world
                                  (cons x y)
                                  (lambda (state)
                                    (state-sense-set! state #t)
                                    state)
                                  (make-state #f #f #f #f))
      (hash-table-update!/default world
                                  (cons (+ x 1) y)
                                  (lambda (state)
                                    (state-thing-set! state maybe)
                                    state)
                                  (make-state #f #f #f #f))
      (hash-table-update!/default world
                                  (cons (- x 1) y)
    ...
    (parameterize ((token "1055751675"))
      (let* ((new-board-id
              (read-url (new-board-url)))
             (new-game-id
              (read-url (new-game-url new-board-id))))
        (parameterize ((board-id new-board-id)
                       (game-id new-game-id))
          (let ((world (make-hash-table))
                (actions (list (forward-url) (left-url) (right-url))))
            (let iter ((status (read-url-json (status-url)))
                       (time 0))
              (if (or (string=? (alist-ref status '|Action Senses|) "Dead")
                      (string=? (alist-ref status '|Action Senses|) "Game Over"))
                  (debug (hash-table->alist world))
                  (let ((position (irregex-match '(: alpha #\( (=> x (+ num)) #\, (=> y (+ num)) #\))
                                                 (alist-ref status '|Actor State|))))
                    (let ((x (string->number (irregex-match-substring position 'x)))
                          (y (string->number (irregex-match-substring position 'y))))
                      (when (member "Breeze" (vector->list (alist-ref status '|Cell Senses|)) string=?)
                        (update-world! world x y state-breeze-set! state-pit-set!))
                      (when (member "Stench" (vector->list (alist-ref status '|Cell Senses|)) string=?)
                        (update-world! world x y state-stench-set! state-wumpus-set!)) 
                      (if (member "Glitter" (vector->list (alist-ref status '|Cell Senses|)) string=?)
                          (debug (read-url-json grab-url))
                          (debug (read-url-json (list-ref actions (random (length actions)))))))
                    (iter (read-url-json (status-url))
                          (add1 time)))))))))
...
** Mon Feb  4 2013
   ...
   - Expanding frontier
** Mon May  6 2013
   Plot the number of possible moves over $n$ games.
** Mon Sep 16 2013
   - Store as trees, e.g. lisp-expressions; not opaque like lambdas,
...
** 6
   ...
   - Domains: enumerable
   - Constraint: $\<scope, rep\>$: $scope$ participates in constraint,
     $rel$ defines the values those relations can take on
   - $X = \{WA, NT, ...\}$
   - $C = \{SA \neq WA\}$
   - Constraint graph
   - Many problems that are intractable for regular state-space search
     can be solved quickly when formulated as a CSP.
   - Precedence constraint
   - Disjunctive constraint
   - CSP formalism
   - Discrete, finite domains
   - Discrete, infinite (integers)
   - Constraint language
   - Linear constraints
   - No algorithm exists for solving general nonlinear constraints on
     integer variables.
   - Continuous domains
   - Operations research
   - Continuous-domain CSPs: linear programming; polynomial time over
     variables
   - Quadratic programming, second-order conic programming
   - Unary constraint
   - Binary constraint
   - Higher-order constraints
   - Global constraints: $Alldiff$
   - Constraint hypergraph: $n$-ary constraints
   - Every finite-domain constraint can be reduced to a set of binary
     constraints if enough auxiliary variables are introduced.
   - Dual-graph transformation: create a new graph in which there will
     be one variable for each constraint in the original graph, and
     one binary constraint for each pair of constraints in the
     original graph that share variables.
   - Preference constraints: costs on invidiual variable assignments
   - Constraint optimization problem
   - Search, constraint propagation: using the constraints to reduce
     the number of legal values for a variable, in turn reduce the
     legal values for anothor, etc.
   - Local consistency
   - Node consistency
     - Since variable (CSP network) node-consistent: if all the values
       in the variable’s domain satisfy the variable’s unary
       constraints.
     - Network: node-consistent, every node node-consistent.
     - Eliminate unary constraints, node-consistency
     - Transform $n$-ary constraints into binary constraints
   - Arc-consistency
     - Variably is arc-consistent if every value in its domain
       satisfies the variable’s binary constrainst.
     - AC-3
       #+BEGIN_SRC org
         ,- (AC-3 csp) \to false, true
         ,  - let
         ,    - queue, queue of arcs, initially all the arcs in csp
         ,      - while queue is not empty
         ,        - (X_i, X_j) \gets (dequeue queue)
         ,        - if (revise X_i X_j)
         ,          - if (positive? (size D_i))
         ,            - return false
         ,            - for each X_k in X_i.neighbors - {X_j}
         ,              - add (X_k, X_i) to queue
         ,      - return true
         ,- (revise csp X_i X_j) -> true iff we revise the domain of X_i
         ,  - revised <- false
         ,  - for x in D_i
         ...
         ,  - return revised
       #+END_SRC
     - After AC-3, we are left with a CSP that is equivalent to the
       original CSP–they both have the same solutions–but the
       arc-consistent CSP will in most cases be faster to search
       because its vriables have smaller domains.
     - Extend arc-consistency to handle $n$-ary cases: hyperarc.
     - $O(cd^3)$ worst-case time
   - Path-consistency
     - Arc-consistency fails to make enough inferences.
     - Map coloring, for instance: satisfy each binary but not the
       network.
     ...
     - PC-2 algorithm
   - $K$-consistency
     - CSP is $k$-consistent if, for any set of $k - 1$ variables and
       for any consistent assignment to those variables, a consistent
       value can always be assigned to any $k$th variables.
     ...
     - $n$-consistency, exponential in time, space.
     - In practice: 2-, 3-consistency
   - Global constraints
     - Higher-order constraints: resource constraints (atmost), bounds
       propagation (upper and lower bounds)
   - Sudoku
     - Empty squares have domain [0 .. 9], pre-filled squares have a
       single-digit domain
     - 27 different Alldiff constraints
     - Alldiff constraints expanded into binary constraints (apply
       AC-3).
     - Eliminate by arc-consistency
     - Solve entire puzzle by AC-3: all variables have domains reduced
       to a single value
     - AC-3 solves only the easiest Sudoku puzzles.
       - [“From that . . .” not “from the fact that . . .”]
     - CSP formalism: each new problem area, define the problem in
       terms of constraints; the general constraint-solving mechanism
       can take over.
   - Solved by inference over constraints; not sufficient, need to
     search: backtracking search algorithms.
   - Depth-limited search; CSP with n variables of a domain size d:
     branching factor at top level $nd$; next: $(n - 1)d$, etc. Tree
     with $n!\dot d^n$ leaves.
   - CSPs: commutativity; order of application no effect on the
     outcome; with this restriction: leaves is $d^n$.
   - Backtracking search
     #+BEGIN_SRC org
       ,- backtracking-search(csp) -> solution, failure
       ,  - return backtrack({}, csp)
       ,- backtrack(assignment, csp) -> solution, failure
       ,  - if assignment is complete
       ,    - return assignment
       ,    - var <- select-unassigned-variable(csp)
       ,      - for each value in order-domain-variables(var, assignment, csp)
       ...
       ,    - return failure
     #+END_SRC
   - Improve performance of CSPs without domain-specific knowledge
     - Which variable assigned next; what order values tried?
     - What inferences performed?
     - Violates a constraint, avoid violating it?
   - Variable and value ordering
     - Next unassigned variable: inefficient
     - Minimum remaining values heuristic (most constrained variable,
       fail-first heuristic)
     - If variable X has no legal values left, will pick and fail
       immediately.
     - Degree heuristic: reduce the branching factor on future choices
       by selecting the variable that is nivolved in the largest
       number of constaints on other unassigned variables.
     - Minimum remaining values with degree heuristic as tie-breaker.
     - Least-constraining value heuristic: tries to leave maximum
       flexibility for subseqent value assignments.
     - Trying to find all solutions, not just one: order doesn’t
       matter; same holds: no solutions.
     - Variable-select fail-first; value-selection fail-last.
   - Interleaving search and inference
     - AC-3 infer reductions in the domain of variables before we
       begin the search.
     - Forward checking: whenever a variable X is assigned, the
       forward-checking process establishes arc consistency for it.
     - MRV heuristic with forward checking: forward checking as an
       efficient way to incrementally compute the information that the
       MRV heuristic needs to do its job.
     - Doesn’t look far enough ahead; MAC (Maintaining Arc
       Consistency) does.
     - Unlike MAC, forward checking does not recursively propagate
       constraints when changes are made to the domains of variables.
   - Intelligent backtracking
     - Chronological backtracking (most recent decision point revisited)
     - Backtrack to variable that might fix the problem
     - Keep track of assignments that are in conflict with some value
       for SA: conflict set
     - Backjumping: most recent assignment in conflict set.
     - Forward checking supplies conflict set with no extra work
     - Every branch pruned by forward checking also pruned by
       backjumping
     - Conflict-directed backjumping: set of preceding variables,
       together with any subsequent variables
     - Constraint learning: finding minumum set of variables from the
       conflict set that causes the problem.
   - Local search
     - Min-conflicts heuristic: select the value that results in the
       minimum number of conflicts with other variables
       #+BEGIN_SRC org
         ,- (min-conflicts csp max-steps) -> solution or failure
         ,  - current <- initial complete assignment for csp
         ,  - for i = 1 to max-steps
         ,    - if current is a solution for csp
         ,      - return current
         ,      - begin
         ,        - var <- randomly chosen conflicted variable for csp.variables
         ,        - value <- the value v for var that minimizes conflicts(var,
         ,          v, current, csp)
         ,        - set var = value in current
         ,  - return failure
       #+END_SRC
     - n-queens, independent of problem size; 1m queens in 50 steps.
     - n-queens easy for local search, densely distributed through
       state space.
     - Local search techniques candidates for CSP; landscape of CSP
       under min-conflicts heuristic has a series of plateux.
     - Tabu search: small list of recently visited states, forbidding
       algorithm to return to those states.
     - Simulated annealing
     - Constraint weighting
     - Local search: used online
   - Structure of the problem
     - Independent subproblems
     - Connected components
     - Constraint graph is a tree when any two variables connected by
       only one path
     - Any tree-structured CSP can be solved in time linear to the
       number of variables.
     - Directed arc consistency (DAC)
     - Topological sort
     - Constraint graphs reduced to trees: removing nodes, collapsing
       nodes
     - Choose e.g. SA, treat the rest of the problem like a tree
     - Cycle cutset
       - 1. Choose a subset S of the CSP’s variables, constraint graph
         becomes a tree: cycle subset
       - 2. For each possible assignment to the variables in S:
         - a. remove inconsistent values from domain
         - b. if has a solution, return with assignment for S
     - Finding smallest cycle cutset NP-hard
     - Cutset conditioning
     - Tree decomposition of graph into connected subproblems
     - Each subproblem solved independently, resulting solutions are
       combined
     - Value symmetry
     - Symmetry-breaking constraint
** 7
   - We develop logic as a genaral class of representations to support
   ...
   - Axiom
   - Tell, ask; inference
   - The conclusion is guaranteed to be correct if the available
     information is correct.
   - Syntax, semantics: truth of each sentence w.r.t. each possible
     world.
   - Model, “possible world”
   - If sentence \alpha is true in model $m$, we say that $m$
     satisfies \alpha; $m$ is a model of \alpha.
   - $M(\alpha)$, the set of all models of \alpha
   - Logical entailment, a sentences follows logically: $\alpha
     \models \beta$; $\alpha \models \beta iff M(\alpha) \subseteq
     M(\beta)$.
   - Stronger assertion: rules out more possible worlds
   - These percepts, combined with an agent’s knowledge of the rules
     of the wumpus world, constitute the KB.
   - The KB is false in models that contradict what the agent knows.
   - \alpha_1 = “There is no pit in [1,2].” \alpha_2 = “There is no
     pit in [2,2].” Hence, KB \models \alpha_1; KB \nmodels \alpha_2.
   - Model checking: $M(KB) \subseteq M(\alpha)$
   - If an inference algorithm $i$ can derive \alpha from KB,
     $KB\vdash_i \alpha$: “\alpha is derived from KB by $i$”, “$i$
     derives $\alpha$ from KB.”
   - Inference algorithm that derives only entailed sentences, sound
     or truth-preserving.
   - Completeness: inference algorithm complete derive any sentence
     that is entailed.
   - If KB is true is the real world, then any sentence \alpha
     derived from KB by a sound inference procedure is also true in
     the real world.
   - “The world is everything that is the case.”
   - Grounding: the connection between logical reasoning processes
     and the real environment in which the agent exists.
   - General rules, learning
   - Syntax, allowable sentences; atomic sentences: a single
     proposition symbol.
   - The semantics for propositional logic must specify how to compute
     the truth value of any sentence, given a model. This is done
     recursively.
   - Atomic sentences: True is true in every model, &c.
     - [The biconditional is like exclusive nor (XAND); $P \to Q$
       unless $P$ and $\neg Q$.]
   - Latin /aut/!
   ...
   - Sentences:
     #+BEGIN_EXAMPLE
       R1: \neg P_{1,1}
       R2: B_{1,1} \equiv (P_{1,2} \vee P_{2,1})
       . . .
       R4: \neg B_{1,1}
     #+END_EXAMPLE
   - With seven symbols, there are $2^7 = 128$ possible models; in
     three of these KB is tree. In those three models, $\neg P_{1,2}$
     is true, hence there is no pit in $[1,2]$. On the other hand
     $P_{2,2}$ is true in two of the three models and false in one, so
     we cannot yet tell whether there is a pit in $[2,2]$.
   - Model checking: enumerating models and showing that the sentence
     must hold in all models.
   - Vs. theorem proving: applying rules of inference directly to the
     sentences in our knowledge base to construct a proof of the
     desired sentence without consulting models.
   - Logical equivalence: $a \equiv b iff a \models b and b \models
     a$.
   - $a \models b iff a \to b$: check $a \to b$ in every model 
   - Satisfiability: sentence true in some model
   - The SAT problem: satisfiability of sentences, NP complete
   - Constraint satisfaction: satisfiability problem
   - $\alpha$ is valid iff $\neg \alpha$ is unsatisfiable
   - $a \entails b$ iff $a \land \neg b$ is unsatisfiable: $reductio
     ad absurdum$
   - One assumes $b$ to be false and shows that this leads to a
     contradiction with know $a$.
   - Proof: chain of conclusions that leads to the desired goal. Modus
     ponens: $a \to b, b; \therefore a$.
   - $a \land b \therefore a$.
   - We found this proof by hand, but we can apply any of the search
     algorithms in Chapter 3 to find a sequence of steps that
     constitutes a proof.
   - Initial state: knowledge base; actions: inference rules applied;
     result: add the sentence in the bottom half of the inference;
     goal: state that contains the sentence we are trying to prove.
   - Finding a proof can be more efficient because the proof can
     ignore irrelevant propositions.
   - Monotonicity: entailed sentences can only increase as information
     is added to the knowledge base.
   - If $KB \entails a$ then $KB \land b \entails a$.
   - Completeness: iterative deepening complete, if inferences rules
     are inadequate, the goal is not reachable.
   - Resolution, yields a complete inference algorithm when coupled
     with any complete search algorithm.
   - $\neg P_{2,2}$ resolves with $P_{2,2}$ in $R_15$ to give the
     resolvent R_16: $P_{1,1} \lor P_{3.1}$.
   - Unit resolution inference rule: complementary literals;
     - Biconditional elimination: rules of inference, propositional
       logic: infer a conditional from a biconditional. $P
       \leftrightarrow Q$ gives $P \to Q$, $Q \to P$.
   - And-elimination
   - Logical equivalence for contrapositives
   - We can apply any of the search algorithms in Chapter 3 to find a
     sequence of stops that constitutes a proof:
     - Initial state :: knowledge base
     - Actions :: Inference rules applied to all the sentences that
                  match the top half of the inference rule
     - Result :: Add the sentence in the bottom half of the inference rule
     - Goal :: State that contains the sentence we are trying to
               prove.
   - Resolution: yields a complete inference algorithm when coupled
     with any complete search algorithm.
   - A literal resolves with the literal to giv eth resolvent . . .
   - Unit resolution inference rule
   - Complementary literals: unit resolution takes clause and literal
     to produce a new clause
   - Resolution-based theorem prover can, for sentences $a$ and $b$ in
     propositional logic, decide whether $a \entails b$.
   - Factoring: removing duplicates
   - Resolution: $P_{1,1} \lor P_{3,1}, \neg P_{1,1} \lor \neg P_{2,2}
     \therefore \P_{3,1} \lor \neg P_{2,2}$
   ...
   - Resolution algorithm: to show that $KB \entails a$, show that $KB
     \land \neg a$ is unsatisfiable.
   - $KB \land \neg a$ is converted into CNF; resolution is applied to
   ...
   - Each pair that contains complementary literals is resolved to
     ...
     already present.
   - Either: there are no new clauses that can be added: $KB$ does not
     entail $a$; or two clauses resolve to yeld the empty clause, in
     which case $KB$ entails $a$.
   - The empty clause, disjunction of no disjuncts, is false.
   - The empty clause arises from resolving two complementary unit
     clauses such as $P$ and $\neg P$.
   - Ground resolution theorem: if a set of clauses is unsatisfiable,
     then the resolution closuer of those clauses contains the empty
     clause.
   - Definite clause: disjunction of literals of which exactly one is
     positive.
   - Horn clause: disjunctions of literals of which at most one is
     positive.
   ...
   - Every definite clause can be written as an implication whose
     premise is a conjuction of positive literals and whose conclusion
     is a single positive literal.
   - In Horn form, the premise is called the /body/ and the conclusion
     is called the /head/.
   - A sentence consisting of a single positive literal is called a
     /fact/.
   - It can be written as $True \to L_{1,1}$.
   - k-CNF: where each clause has at most $k$ literals.
   ...
   - $pl-fc-entails?(KB, q)$ determines if a single proposition symbol
     $q$ is entailed by a knowledge base of definite clauses.
   - It begins from known facts; if all the premises of an implication
     are known, then its conclusion as added to the set of known facts.
   - Continues until $q$ is added or no further inferences can be made.
   - Backward chaining: goal-directed reasoning; and-or graph search.
   - Forward chaining: data-driven reasoning; Wumpus agent might
     =tell= its percepts to the knowledge base using an incremental
     forward-chaining algorithm in which new facts can be added to the
     agenda to initiate now inferences.
   - Checking satisfiability: the SAT problem.
   - Based on backtracking, based on hill-climbing
   - Connection between finding a satisfying model for a logical
     sentence and finding a solution for a constraint satisfaction problem
   - So many combinatorial problems in computer science can be reduced
   ...
   - Scale SAT solvers:
     - Component analysis (Tasmania)
     - Variable and value ordering (degree heuristic)
     - Intelligent backtracking (conflict clause learning)
     - Random restarts
     - Clever indexing (set of clauses in which $X_i$ appears as a
       positive literal)
   - =walk-sat=: picks an unsatisfied claue and picks a symbol to
     flip; chooses between: min-conflict and random-walk for flipping
     a symbol
   - When =walk-sat= returns a model: satisfiable; failure:
     unsatisfiable or not enough time.
   - =walk-sat= is useful when we expect a solution to exist.
   - n-queens is underconstrained
   - An underconstrained problem is one with relatively few clauses
     constraining the variables.
   - Overconstrained: many clauses relative to the number of variables
     and is likely to have no solutions
   - $CNF_k(m, n)$: $k$-CNF cause whith $m$ clauses and $n$ symbols.
   - Satisfiability threshold conjecture: for every $k \geq 3$, there
     is a threshold ratio $r_k$ such that, as $n$ goes to infinity,
     the probability that $CNF_k(n, rn)$ is satisfiablie becomes 1 for
     all values fo $r$ below the threshold, and 0 for all values above.
   - The probability drops fairly sharply around $m/n = 4.3$.
   - Cliff stays in place for $k = 3$ and gets sharper and sharper as
     $n$ increases.
   - Deduce the state of the world given its percept history: writing
     down logical model of the effects of actions
   - Agent can keep track of the world efficiently without going back
     into the percept history for each inference.
   - Use logical inference to construct plans that are guaranteed to
     achieve its goals.
   - Logical agent operates by deducing what to do from a knowledge
     base of sentences about the world.
   - Knowledge base composed of axioms and percepts.
   - Deducing the current state of the wumpus world: where am I, is
     that square safe, &c.
   - Agent knows rules for pits and wumpi
   - Percept asserts something only about the current time
   - =kb-agent=, =make-percept-sentence=
   - $stench^4$, $\neg stench^3$
   - $L^0_{1,1}$: agent is in square $[1,1]$ at time 0.
   - Fluent: aspect of world that changes (state variable)
   - Atemporal variables
   - Location fluent: time step $t$ and square $[x, y]$:
     - $L_{x,y}^t \to (Breeze^t \equiv B_{x,y})$
     - $L_{x,y}^t \to (Stench^t \equiv S_{x,y})$
   - Fluents change as a result of actions: transition model
   - Proposition symbols, occurrences of actions: $forward^0$
   - Effect axioms: specify the outcome of an action at the next time
     step:
     - $L_{1,1}^0 \land east^0 \land forward^0 \to (L_{2,1}^1 \land
       \neg L_{1,1}^1$
   - E.g. =ask(kb, L_{2,1}^1)= is true; =ask(kb, arrow^1)= is false:
     effect axiom fails to state what remains unchanged as the result
     of an action.
   - The /frame problem/; add frame axioms explicitly asserting all
     the propositions that remain the same:
     - $forward^4 \to (havearrow^t \equiv havearrow^{t+1})
     - $forward^4 \to (wumpusalive^t \equiv wumpusalive^{t+1})
     - . . .
   - Frame axioms of size $O(mn)$
   - Representational frame problem
   - Locality: action changes no more than some small number $k$ of
     fluents
   - Transition model with $O(mk)$ axioms
   - Inferential frame problem: project forward the results of a $t$
     step plan: $O(kt)$ rather than $O(nt)$.
   - Changing one’s focus from writing axioms about actions to writing
     axioms about fluents
   - Successor-state axiom: $F^{t+1} \equiv ActionCausesF^t \lor (F^t
     \land \neg ActionCausesNotF^t$
   - $HaveArrow^{t+1} \equiv (HaveArrow^t \land \neg Shoot^t)
   - $L^{t+1}_{1,1} \equiv (L^t_{1,1} \land (\neg Forward^t \lor
     Bump^{t+1})) \lor (L^t_{1,2} \land (South^t \land Forward^t))
     \lor (L^t_{2,1} \land (West^t \land Forward^t))$
   - $OK^t \equiv \neg P_{x,y} \and \neg (W_{x,y} \land WumpusAlive^t)$
   - Qualification problem: specifying enumerable exceptions that an
     action has a certain effect.
   - Hybrid agent: deduce aspects of the state of the world:
     condition-action rules, problem solving algorithms from chs. 3
     and 4.
   - Agent maintains and updates a knowledge base as well as a current
     plan.
   - Initial knowledge base contains atemporal axioms: those that
     don’t depend on $t$; and relate the breeziness of squares to the
     presence of pits.
   - At each $t$, new percept sentence added along with all the axioms
     that depend on $t$, such as the successor-state axioms.
   - Agent uses logical inference, by =asking= questions of the
     knowledge base to work out which square are safe and have yet to
     be visited.
   - Constructs plan based on a decreasing priority of goals.
   - If glitter, plan to grab gold, follow route back, climb out.
   - Otherwise, no current plan, program plans a route to the closest
     safe square not visited yet.
   - Route planning is done with $A^*$, not with =ask=.
   - If there are no safe squares, try to make a safe square by
     shooting at one of the possible wumpus locations: asking where
     $ask(KB, \neg W_{x,y})$ is false, where it is not known that
     there is not a wumpus.
   - =plan-shot= uses =plan-route= to plan a sequence of actions that
     will line up the shot. If it fails, looks for a square that is
     not provably unsafe: $ask(KB, \neg OK^t_{x,y})$ is false. If no
     square, retreats to $[1, 1]$ and leaves.
   - As time goes by, computational expense in =ask= goes up.
   - Constant update time independent of $t$; cache the results of
     inference.
   - Belief state: updating the belief state as new percepts arrive:
     state estimation: $WumpusAlive^1 \land L^1_{2,1} \land B_{2,1}
     \land (P_{3,1} \lor P_{2,2})$.
   - If $n$ fluent symbols, $2^n$ physical states.
   - Belief states, powerset of all subsets: $2^2^n$ belief states.
   - Approximate state-representation: belief states as conjunctions
     of literals, that is, 1-CNF formulas.
   - Tries to prove $X^t$ and $\neg X^t$ for each symbol $X^t$ (as
     well as each atemporal symbol whose truth value is not yet
     known), given the belief state at $t - 1$.
   - 1-CNF, conservative approximation; outer envelope around exact
     belief state.
   - Uses A* to make plans.
   - Signifcant difference between the requirements for entailment (as
     tested by Ask) and those for satisfiability.
   - Needs preconditions
   - Action exclusion axioms
   - For any practical agent, these phrases have to be implemented by
     code that generates instances of general sentence schema
     automatically for insertion into the knowledge base.
*** Shannon, A Mathematical Theory of Communication
    - The fundamental problem of communication is that of reproducing
      ...
      at another point. Frequencly the messages have meaning; that is
      they refer to or are correlated according to some system with
      certain physical or conceptual entities. The significant aspect
      is that the actual message is one selected from a set of
      possible messages.
    - Logarithm: practical, intuitive, suitable.
    - $C = log X_0$, where $C$ is the capacity of a discrete channel.
    - We have represented a discrete information source as a Markoff
      process.
    - H should be continuous in the p_i (p. 392).
    - If all the p_i are equal, p_i = 1 / n, H should be a monotonic
    ...
    - If a choice be broken down into two successive choices, the
      ...
      of H.
    - $H = -K\Sum_{i=1}^n p_i log p_i$, where K is a positive
      constant.
    - $H$ is the $H$ in Boltzmann’s famous $H$ theorem.
      - Entropy related to the number of distinguishable macrostates.
    - $H = 0$ iff all the p_i but on are zero (p. 394).
    - For a given n, H is a maximum and equal to log n when all the
      p_i are equal.
    - $H = \Sum P_i H_i$
** 8
   - What programming languages lack is any general mechanism for
     deriving facts from other facts.
   - Declarative nature of propositional logic: knowledge and
     inference are separate; inference is domain-independent.
   - Data structures in programs . . . lack the expressiveness
     required to handle partial information.
   - Propositional logic . . . has sufficient expressive power to deal
     with partial information, using disjunction and negation.
   - Compositionality: meaning of a sentence is a function of the
   ...
   - For nondeductive tasks such as learning from experience, outcomes
     ...
     - [Never got a chance to do walksat.]
   - Adopt the foundation of propositional - a declarative,
     compositional semantics that is context-independent and
     unambigiuous - and build a more expressive logic on that
     foundation, borrowing representatial ideas from natural language
     while avoiding its brawbacks.
   - Objects: nouns
   - Relations: verbs; unary relations, i.e. properties
   - Functions: relations in which there is only one value for a given
     input.
   - First-order logic is built around objects and relations.
   - First-order logic can express facts about some or all of the
     objects in the universe.
   - Primary difference between propositional and first-order logic
     lies in the ontological commitment made by each language.
   - Ontological commitment expressed through the nature of the formal
     models with respect to which the truth of sentences is defined.
   - Propositional: each model assigns true or false to each
     proposition symbol.
   - First order logic assumes: the world consists of objects with
     certain relations that do or do not hold.
   - Special-purpose logics make still further ontological
     commitments: temporal logic assumes that facts hold at particular
     times and that those times are ordered.
   - Special-purpose logics give certain kinds of objects (and the
     axioms about them) first-class status within the logic, rather
     than simply defining them within the knowledge base.
   - Higher-order logic views the relations and functions referred to
     by first-order logic as objects in themselves.
   - Epistemological commitments: possible states of knowledge that
     it allows with respect to each fact.
   - Probability theory, on the other hand, can have any degree of
     belief.
     - Don’t confuse the degree of belief in probability theory with
       degree of truth in fuzzy logic.
   - Models for first-order logic
     - Domain model is the set of objects or domain elements it
       contains.
     - A relation is just the set of tuples of objects that are
       related.
     - Brotherhood-relation
       - {<Richard, John>, <John, Richard>}
     - Unary relations, e.g. person
       - {<Richard>, <John>}
     - Functions, e.g. left-leg
       - <Richard> \to Richard’s left leg
       - <Crown> \to No left leg
   - Symbols
     - Constant symbols, predicate symbols, function symbols
     - Predicate and functions symbols have arity
     - As in propositional, every model must provide the information
       required to determine if any given sentence is true or false.
     - Each model includes an interpretation that specifies exactly
       which objects, relations and functions are referred to by the
       constant, predicate, function symbols.
     - Interpretation under which e.g. Richard refers to the crown.
     - Entailment, validity defined in terms of all possible worlds.
     - When there are more objects than constant symbols, some of the
       objects will have no names.
     - Because the number of possible models is unbounded, checking
       entailment by the enumeration of all possible models is not
       feasible for first-order logic (unlike propositional logic).
   - Terms
     - Term is a logical expression that refers to an object.
     - Constant symbols are terms.
     - Complex terms, e.g. $LeftLeg(John)$, formed by a function
       symbol and arguments.
     - $f(t_1, ..., t_n)$, where $f$ refers to a function in the
       model; the argument terms refer to objects in the domain; the
       term as a whole refers to the object.
     - Complex terms are complicated names, not subroutines that
       return a value.
     - Reason about left-legs without defining left-legs; nothing a
       subroutine could do.
   - Atomic sentences
     - Atom: predicate symbol optionally followed by a list of terms,
       e.g. $Brother(Richard, John)$, $Married(Father(Richard),
       Mother(John))$.
     - An atomic sentence is true in a given model if the relation
       referred to by the predicate symbol holds among the objects
       referred to by the arguments.
   - Complex sentences
     - Logical connectives
       - $(not (brother (left-leg richard) john))$
       - $(and (brother richard john) (brother john richard))$
   - Quantifiers
     - Express properties of entire collections of objects, instead
       of enumerating objects by name.
     - Universal, existential
     - Universal
       - $(all (x) (=> (king x) (person x)))$
       - $x$ is called a variable.
       - A term with no variables: ground term
       - $\forall x P$ is true for every object $x$.
       - Also, extended interpretations constructed from the
         interpretation given in the model, where each extended
         interpretation specifies a domain object to which $x$ refers.
       - End up asserting the conclusion of the rule just for those
         objects for whom the premise is true and saying nothing at
         all about those invidiuals for whom the premise is false.
     - Existential
       - E.g. $\exists x Crown(x) \land OnHead(x, John)$
       - $\exists x P$, $P$ is true for at least one object $x$;
         i.e., is true in at least one extended interpretation that
         assigns $x$ to a domain element.
       - $\exists!$: there exists exactly one.
       - Just as \to is the natural connective to use with $\forall$,
         $\land$ is the natural connective to use with $\exists$.
     - Nested quantifiers
       - $(all (x y) (=> (brother x y) (sibling x y)))$
       - $(all (x) (all (y) ...))$
       - $(all (x y) (<=> (sibling x y) (sibling y x)))$
       - $(all (x) (or (crown x) (exists (x) (brother (richard x)))))$
         - Innermost
     - Relation between $\exists$ and $\forall$
       - De Morgan’s, since $\forall$ is a conjunction over the
         universe of objects and $\exists$ is a disjunction.
       - $(<=> (all (x) (not P)) (not (exists (x) P)))$
       - $(<=> (not (all (x) P)) (exists (x) (not P)))$
       - $(<=> (all (x) P) (not (exists (x) (not P))))$
       - $(<=> (exists (x) P) (not (all (x) (not P))))$
       - “Readibility more import than parsimony.”
   - Equality
     - $Father(John) = Henry$
     - $(exists (x y) (and (brother x richard) (brother y richard)
       (not (= x y))))$
     - $(and (brother john richard) (brother geoffrey richard) (not
       (= john geoffrey)) (all (x) (=> (brother x richard) (or (= x
       john) (= x geoffrey)))))$
   - Database semantics (distinguished from first-order semantics)
     - Unique-names assumption
       - Every constant symbol refer to a distinct object
     - Closed-world assumption
       - Atomic sentences not known to be true are false
     - Domain closure
       - Each model contains no more domain elements than those
         named by the constant symbols
   - Sentences add to KB by =tell=: assertions
     - =(tell kb (king john))=
     - =(tell kb (person richard))=
     - =(tell kb (all (x) (=> (king x) (person x))))=
   - Ask questions of the knowledge base using =ask=; questions
     with =ask= are queries or goals.
   - Any query entailed by the knowledge base should be answered
     affirmatively.
   - =(ask kb (exists (x) (person x))) => #t=.
   - =(ask-vars kb (person x)) => <stream of vars>=
   - Substitution or binding list: =((x . john) (x . richard))=
   - =ask-vars= reserved for knowledge bases consisting solely of
     [[http://en.wikipedia.org/wiki/Horn_clause][Horn clauses]], every way of making the query true will bind the
     variables to specific values.
   - That is not the case with FOL: =(or (king john) (king
     richard))=: no binding $x$ for =(exists (x) (king x))=, even
     though true.
   - Using first-order logic
     - ~(for-all (m c) (<=> (= (mother c) m) (and (female m) (parent m c))))~
     - =(for-all (x) (<=> (male x) (not (female x))))=
     - =(for-all (p c) (<=> (parent p c) (child c p)))=
     - Axiom of the kinship domain: basic factual information, from
       which useful conclusions are derived.
     - Definitions bottom out at a basic set of predicates.
     - Not all logical sentences about a domain are axioms; some are
       theorems: they are entailed by the axioms.
     - Logical point of niew, KB need contain only axioms but no
       theorems. Practically, theorems are essential to reduce the
       cost of deriving new sentences.
     - Without them, physicist has to rederive calculus.
       - [Feynman: Greek vs. Babylonian mathematics.]
     - Substitution/binding-list
     - Not all axioms are definitions
     - First-order logic allows us to make use of the $Person$
       predicate without completely definiting it.
     - Write partial specifications of properties that every person
       has and properties that make something a person.
     - Axioms: plain facts: $Male(Jim)$.
   - Numbers, sets, lists
     - 0 and successor
     - Peano axioms
       - =(nat-num 0)=
       - =(for-all (n) (=> (nat-num n) (nat-num (s n))))=
     - =(for-all (n) (not (= 0 (s n))))=
     - =(for-all (n m) (=> (not (= m n)) (not (= (s m) (s n)))))=
     - =(for-all (m) (=> (nat-num m) (= (+ 0 m) m)))=
     - =(for-all (m n) (=> (and (nat-num m) (nat-num n)) (= (+ (s m)
       n) (s (+ m n)))))=
     - Thus, tho whole of number theory (including cryptography) can
       be built up from one constant, one function, one predicate and
       four axioms.
     - Empty set, union, intersection
     - =(for-all (<=> (set s) (or (= s {}) (exists (x s2) (and (set
       s2) (= s (cons x s2)))))))=
     - =(not (exists (x s) (= (cons x s) nil)))=
   - Wumpus
     - =(percept (stench breeze glitter none none) 5)=
     - =(turn right)=, =forward=
     - =(ask-vars (exists a (best-action a 5)))=
     - =(for-all (=> (t s g m c) (percept (s breeze g m c) t) (breeze t)))$
       - Reasoning process called perception.
       - Quantification over time t.
       - Reflex behavior: =(for-all t (=> (glitter t) (best-action
         grab t)))=.
       - =(for-all (x y a b) (<=> (adjacent ((x y) (a b)) (or (and (= x a)
         (or (= y (- b 1)) (= y (+ b 1)))) (and (= y b) (or (= x (-
         a 1)) (= x (+ a 1))))))))=
       - =(at (agent s t))=
       - =(for-all (t) (at (wumpus (2 2) t)))=
       - =(for-all (x s1 s2 t) (=> (and (at x s1 t) (at x s2 t)) (= s1 2)))=
       - =(for-all (s t) (=> (and (at (agent s t)) (breeze t))))=
       - =(for-all (<=> (breezy s) (exists (r) (and (adjacent r s)
         (pit r)))))=
       - =(for-all (t) (<=> (have-arrow (+ t 1)) (and (have-arrow t)
         (not (action (shoot t))))))=
   - Knowledge engineering
     - Special-purpose knowledge bases
     - General-purpose knowledge bases, e.g. NLP
     - Identify task
       - Will sensor facts include the current location?
       - Analogous to PEAS
     - Assemble relevant knowledge
       - Scope
     - Vocabulary of predicates, functions, constants
       - Like programming style, this can have a significant impact on
         the eventual success of a project
...
*** Testing mini-kanren
    #+BEGIN_SRC scheme
      ...
      (define (spouse x y)
        (conde ((== x 'george) (== y 'mum))
               ((== y 'george) (== x 'mum))))
      
      (define (parent p c)
        (conde ((== p 'george) (== c 'elizabeth))
               ((== p 'mum) (== c 'elizabeth))
               ((== p 'george) (== c 'margaret))
               ((== p 'mum) (== c 'margaret))
               ((== p 'elizabeth) (== c 'charles))
               ((== p 'elizabeth) (== c 'anne))
               ((== p 'elizabeth) (== c 'andrew))
               ((== p 'elizabeth) (== c 'edward))
               ((== p 'philip) (== c 'charles))
               ((== p 'philip) (== c 'anne))
               ((== p 'philip) (== c 'andrew))
               ((== p 'philip) (== c 'edward))))
      
      (define (grand-parent pp c)
        (fresh (p)
          (parent pp p)
          (parent p c)))
      ...
      (run* (r)
        (fresh (x y)
          (conde ((spouse x y) (== r (cons x y))))))
      
      (run* (r)
        (fresh (p c)
          (conde ((parent p c) (== r (cons p c))))))
      
      (run* (r)
        (fresh (pp c)
          (conde ((grand-parent pp c) (== r (cons pp c))))))
      
      (define-syntax spouses
        (ir-macro-transformer
         (lambda (expression inject compare)
           (let ((spouses (cdr expression)))
             `(lambda (x y)
                (conde ,@(append-map
                          (match-lambda ((x y)
                                    (list `((== x ',x) (== y ',y))
      ...
      (define-syntax parents
        (ir-macro-transformer
         (lambda (expression inject compare)
           (let ((parent-children (cdr expression)))
             `(for-each (match-lambda ((parents children) (debug parents children)))
                ,parent-children)
             `(lambda (p c)
                (conde ,@(append-map
                          (match-lambda ((parents children)
      ...
      (define (grand-parent parent pp c)
        (fresh (p)
          (parent pp p)
          (parent p c)))
      
      (define (sibling parent x y)
        (fresh (p)
          (parent p x)
          (parent p y)))
      
      (let ((spouse (spouses (george mum)
                             (elizabeth philip)
                             (spencer kydd)))
            (parent (parents ((george mum) (elizabeth margaret))
        ...
        (debug
         ...
         (run* (r)
           (fresh (gp c)
             (grand-parent parent gp c)
             (== r (cons gp c))))
         (run* (r)
           (fresh (s1 s2)
             (sibling parent s1 s2)
             (== r (cons s1 s2)))))) 
...
** Larrañaga, Genetic algorithms
   ...
   - Path representation: list of $n$ cities; classical operators also
     ...
     - Partially mapped crossover (PMX): selects two random cut points
       . . .
* TODOs
** TODO Wumpus-world web service
   “Give me a 3x3 . . .”
** DONE 6.1
   CLOSED: [2013-06-28 Fri 01:14]
   [[http://en.wikipedia.org/wiki/Graph_coloring#Bounds_on_the_chromatic_number][Chromatic number]]: clique number is the lower bound; [[http://en.wikipedia.org/wiki/Graph_coloring#Chromatic_polynomial][chromatic
   polynomial]]: number of ways to color a graph using no more than $k$
   colors: “the chromatic polynomial is a function P(G, t) that counts
   the number of t-colorings of G.”

   Calculating by [[http://en.wikipedia.org/wiki/Chromatic_polynomial#Deletion.E2.80.93contraction][deletion-contraction]]: “A recursive way of computing
   the chromatic polynomial is based on edge contraction: for a pair of
   vertices $u$ and $v$ the graph $G/uv$ is obtained by merging the two
   vertices and removing any edges between them.”

   #+BEGIN_SRC scheme :comments link :tangle backtracking.scm :shebang #!/usr/bin/env chicken-scheme
   (use debug
        define-record-and-printer
        matchable
        srfi-1
   ...
   CSP has variables which in turn have values? There is a thing called
   “assigment,” really a lookup-table?

   Is it the case that merely searching depth-firstly suffices for
   enumeration?

   $CSP$ has $X$, $D$, $C$: variables, domains, constraints.

   Implementing e.g. AC-3 will help clarify the nature of a CSP, I
   think. By arcs, it’s doing bidirectional constraints? I.e., for a
   given constraint $C$, have to add each side of the scope as whence?
   Seems like it.

   Yeah: “To make X arc-consistent with respect Y, we reduce X’s domain
   to {0, 1, 2, 3}. If we also make Y arc-consistent with respect to X,
   then Y’s domain becomes {0, 1, 4, 9} and the whole CSP is
   arc-consistent” (p. 208).

   If constraints are hash-tables, need to normalize the scope for
   lookup; alphabetize?

   Let’s restrict ourselves to binary constraints.

   Let’s furthermore add bidirectional constraints.

   An [[http://en.wikipedia.org/wiki/Backtracking][alternative]] to copying data structures:

   #+BEGIN_QUOTE
   In addition to retaining minimal recovery values used in backing up,
   backtracking implementations commonly keep a variable trail, to
   record value change history. An efficient implementation will avoid
   ...
   An alternative to the variable trail is to keep a timestamp of when
   the last change was made to the variable. The timestamp is compared
   to the timestamp of a choice point. If the choice point has an
   associated time later than that of the variable, it is unnecessary
   to revert the variable when the choice point is backtracked, as it
   was changed before the choice point occurred.
   #+END_QUOTE

   It doesn’t suffice merely to copy; we need to remove assignments and
   inferences to do exhaustive search, I think.

   See [[http://see.stanford.edu/materials/icspacs106b/H19-RecBacktrackExamples.pdf%0A][here]].
** TODO Implement Scrabble server over REST with polling.
   - Pass possible word moves with score, geometry
   - Generate all possible opponent-moves for each 61 choose 7 rack.
   - Weighted
   - Specify which opponent-racks to evaluate
   - If I’ve played word $w$; evaluate the possible moves against some
     opponent rack $R$.
   - As part of the game state, send the remaining letters.
   - Some fixed number of opponent racks to evaluate?
   - As a parameter: how many racks allowed to query per turn.
   - For each rack, get back a list of words sorted by score.
   - Code locally for generating playable sets: heuristics (“I’m only
     going to generate four letter words this turn,” i.e. operate on
     some subset of the dictionary.)
   - Local search: lexicon (access full scrabble lexicon);
     board-state; rack.
   - Communication with the server at that point: querying for the
     board state; including which in bag, which in rack; whose turn it
     i.s
...
*** CANCELED Scoring
    ...
    #+END_QUOTE
*** CANCELED Bonus, all seven; end game
    CLOSED: [2013-05-01 Wed 16:47]
    ...
    #+END_QUOTE
*** CANCELED Exchange tiles
    CLOSED: [2013-05-01 Wed 16:47]
*** CANCELED GADDAGs in a database of some kind
    CLOSED: [2013-05-01 Wed 16:47]
*** CANCELED Blanks
    CLOSED: [2013-05-01 Wed 16:47]
*** CANCELED Randomly pull off top candidates
    CLOSED: [2013-05-01 Wed 16:47]
