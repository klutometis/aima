...
* TODO 5.4
  ...
  #+BEGIN_SRC scheme
    ...
    (define terminals (make-parameters (make-hash-table)))
    (define non-terminals (make-parameters (make-hash-table)))
    
    (define (terminal letter)
      (hash-table-update!
       (terminals)
       letter
       identity
       (make-letter letter #f)))
    
    (define (non-terminal letter)
      (hash-table-update!
       (non-terminals)
       letter
       identity
       (make-letter letter #t)))
    
    (define (integrate-fixes dag prefix suffix)
      (debug 'integrate-fixes prefix suffix)
      (let* ((prefix (reverse prefix))
             (characters (if (null? suffix)
                          prefix
                          (append (append prefix (list sentinel))
                                  suffix))))
        ;; Set the result of this fold to be terminal.
        (fold (lambda (character dag)
                (let ((subdag (update-hash-table!/default
                               dag
                               character
                               (lambda (subdag)
                                 (hash-table-set!
    ...
    (update-dag! (make-hash-table) "harro")
  #+END_SRC
