...
* TODO 3.7
  #+BEGIN_SRC scheme :tangle 3.7.scm :shebang #!/usr/bin/env chicken-scheme
    ...
    ;;; Binary heap package is weird; took pains to do it functionally. We
    ;;; just need a straight-forward, destructive one. Hmm: thanks the
    ;;; Olympioi we wrote one for CLRS. Is it servicable?
    (define (make-heap)
      (make-binary-heap
       (lambda (c1 c2)
         (cond ((> c1 c2) -1)
    ...
    (define (uniform-cost start end neighbors)
      (let ((heap (make-heap)))
        (search start
                ...
                neighbors
                (lambda () heap)
                (lambda (frontier node)
                  (if (zero? (heap 'size))
                      (set! heap (make-heap)))
                  (set! heap ((heap 'put) (node-path-cost node) node)))
                (lambda (frontier)
                  (let ((max (cdr ((heap 'get-max)))))
                    (set! heap ((heap 'delete-max)))
                    max))
                (lambda (frontier)
                  (heap 'empty?)))))
      ...
      (let ((start (tessellation-start tessellation))
        ...
        (let ((breadth-first (breadth-first start end neighbors))
          ...
          (plot-tessellation/animation
           tessellation
           (predecessor-path uniform-cost)
           "Uniform-cost"
           "uniform-cost")
          (plot-tessellation/animation
           tessellation
           (predecessor-path depth-first)
           "Depth-first"
           "depth-first")
          (plot-tessellation/animation
           tessellation
           (predecessor-path breadth-first)
           "Breadth-first"
           "breadth-first"))))
