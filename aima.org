...
** TODO a
   ...
   - Initial state :: A friend a each city $n(\{i, j\})_0$, $P(\{i,
                      j\})$ are empty.
   - Actions :: Each friend moves to a neighboring city (or stays
                put).
   - Transition model :: Each friend moves to a neighboring city (or
        stays put).
   - Goal test :: $n(i) = n(j)$, the two friends are in the same city.
   - Path cost :: $max(T(i_n, i_{n+1}), T(j_n, j_{n+1})$, where $T$ is
                  the time required to move from one city to the next.
