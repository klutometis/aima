   ...
   #+BEGIN_SRC scheme
     ...
     (define (display-graph graph)
       ...
       (hash-table-walk
        graph
        (lambda (from to)
          (for-each (lambda (to) (format #t "~a->~a;" from to)) to)))
       (display "}\n"))
     
     ;;; See this note on cardinality, by the way
     ;;; (<http://en.wikipedia.org/wiki/Cardinality>): "The cardinality of
     ;;; a set A is usually denoted | A |. Alternatively, the cardinality
     ;;; of a set A may be denoted by n(A), A, or # A."
