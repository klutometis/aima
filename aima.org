...
** DONE d
   ...
   #+BEGIN_SRC scheme :tangle tsp.scm
     ...
     (define (write-tree-as-dot tree)
       ;; (pp (hash-table->alist tree) (current-error-port))
       (display "digraph G {graph[splines=true];")
       (for-each (lambda (node) (format #t "\"~a\";" node))
         (hash-table-keys tree))
       (hash-table-walk tree
                        (lambda (from tos)
                          (for-each (lambda (to)
                                      (format #t "\"~a\" -> \"~a\"[label=\"~a\"];"
                                              from to (point-distance from to)))
                            tos)))
       (display "}"))
     ...
     (define (nearest-neighbor from points)
       (let ((neighbors (make-min-heap)))
         (for-each (lambda (to) (heap-insert! neighbors (point-distance from to) to))
           points)
         (heap-extract-extremum! neighbors)))
     
     (define (graph-tour tour)
       (display "x,y,xend,yend\n")
       (hash-table-walk
        tour
        (lambda (from to)
          (format #t "~a,~a,~a,~a~%"
               ...
               (hash-table-map
                ...
                (lambda (from to)
                  (vector (point-x from)
                          ...
                          (point-x to)
                          (point-y to)))))
              (tour (R (data.frame (t (matrix ,(list->vector tour) nrow: 4))))))
         (R (colnames<- ,tour (c "x" "y" "xend" "yend")))))
     
     (define (tour->vectors tour)
       (let ((tour
              (hash-table-map
               tour
               (lambda (from to)
     ...
     ;;; Let's define a ggplot abstraction, and even consider adding it to
     ;;; R. It might actually be useful here to be able to start and stop R
     ;;; sessions so that we're starting fresh. This whole thing depends on
     ;;; mutation, after all, since I can't just pass objects to ggplot:
     ;;; there's language tricks afoot.
     (define (graph-tour strategy output tour)
       (let ((data (tour->data tour))
             (tour-length (tour-length tour)))
         (R (library "ggplot2"))
         (call-with-values (lambda () (tour->vectors tour))
           (lambda (x y xend yend)
             (R (png ,output height: 768 width: 1024 type: "cairo"))
             (R (plot ,x ,y))
             (R (segments ,x ,y ,xend ,yend))
             (R (dev.off))))))
     
     (define (graph-tour-with-tree strategy output tour tree points)
       (let-values (((x y xend yend) (tour->vectors tour))
                    ((x-tree y-tree xend-tree yend-tree) (tree->vectors tree)))
         (R (png ,output height: 768 width: 1024 type: "cairo"))
         (R (plot ,(list->vector (map point-x points))
                  ,(list->vector (map point-y points))))
         (when (positive? (vector-length x))
           (R (segments ,x ,y ,xend ,yend col: 1))
           ;; Last segment isn't necessarily the last on the path, since
           ;; we're using hash-tables.
           ;; (R (segments ,(vector-ref x (- (vector-length x) 1))
           ;;              ,(vector-ref y (- (vector-length y) 1))
           ;;              ,(vector-ref xend (- (vector-length xend) 1))
           ;;              ,(vector-ref yend (- (vector-length yend) 1))
           ;;              col: 2))
           )
         (when (positive? (vector-length x-tree))
           (R (segments ,x-tree ,y-tree ,xend-tree ,yend-tree col: 2)))
         (R (dev.off))))
     
     (define graph? (make-parameter #t))
     
     (define (A*-tour start graph points)
               ...
               (reverse
                (map node-state
                     (A* start
                         ...
                         point-distance
                         (lambda (node) (= (length (predecessor-path node)) (length points)))
                         (lambda (node)
                           (let* ((visited-points (map node-state (predecessor-path node)))
                                  (unvisited-points (lset-difference eq? points visited-points))
                                  (unvisited-tree (minimum-spanning-tree (complete-graph unvisited-points))))
                             ;; (debug (hash-table->alist unvisited-tree)
                             ;;        (make-tour visited-points)
                             ;;        (length (hash-table-keys unvisited-tree))
                             ;;        (length visited-points))
                             ;; (when (> (length visited-points) 1)
                             ;;   (graph-tour (make-tour visited-points)
                             ;;               "A*"
                             ;;               (begin (set! n (+ n 1))
                             ;;                      (make-pathname dir (number->string n) ".png"))
                             ;;               unvisited-tree
                             ;;               (make-tour visited-points)))
                             ;; (when (and (graph?)
                             ;;            (= (length (hash-table-keys unvisited-tree)) 5))
                             ;;   (graph-tour (make-tour visited-points)
                             ;;               "A*"
                             ;;               "a-star-with-mst.png"
                             ;;               unvisited-tree)
                             ...
                             (let ((point (node-state node)))
                               (let ((nearest-neighbor
                                      (if (null? unvisited-points)
                                          0
                                          (point-distance
                                           point
                                           (nearest-neighbor point unvisited-points))))
                                     (nearest-start
                                      (if (null? unvisited-points)
                                          0
                                          (point-distance
                                           start
                                           (nearest-neighbor start unvisited-points)))))
                                 (+ nearest-neighbor
                                    (* (tree-cost unvisited-tree) 2)
                                    nearest-start)))))))))))
   ...
   #+BEGIN_SRC scheme :tangle 3.30.scm :shebang #!/usr/bin/env chicken-scheme
     ...
     (let* ((graph (complete-graph (make-random-points 20)))
            ...
            (start (car points)))
       (let* ((tour (A*-tour start graph points))
              (tour (make-tour tour)))
         ;; (graph-tour tour)
         ;; (debug start tour (hash-table->alist (make-tour tour)))
         ;; (with-output-to-file "tour.csv"
         ;;   (lambda () (graph-tour (make-tour tour))))
         (graph-tour "A*" "harro.png" tour)))     
