...
** TODO d
   ...
   #+BEGIN_SRC scheme
     ...
     ;;        (graph (complete-graph points)))
     ;;   (debug (hash-table->alist graph))
     ;;   (let ((heap (make-min-heap)))
     ;;     (hash-table-walk graph
     ;;                      (lambda (from tos)
     ;;                        (for-each (lambda (to)
     ;;                                    (debug (point-distance from to) from to)
     ;;                                    (heap-insert! heap (point-distance from to) (cons from to)))
     ;;                          tos)))
     ;;     (do ((heap heap))
     ;;         ((heap-empty? heap))
     ;;       (let ((from-to (heap-extract-extremum! heap)))
     ;;         (debug (point-distance (car from-to) (cdr from-to)))))))
     ...
     ;;; Union has to set all elements!
     (define (set-union! set e1 e2)
       (hash-table-set! set e2 (hash-table-ref set e1)))
     
     (define (minimum-spanning-tree graph)
       (let ((set (make-disjoint-set values eq?))
             (tree (make-hash-table)))
         (for-each (lambda (node) (disjoint-set:make set node))
         ...
         (let ((edges (make-min-heap)))
           ...
           (do ((edges edges))
             ...
             (let ((edge (heap-extract-extremum! edges)))
               (let ((from (car edge))
                     (to (cdr edge)))
                 (unless (eq? (disjoint-set:find set from)
                              (disjoint-set:find set to))
                   ;; (biconnect! tree from to)
                   (connect! tree from to)
                   (disjoint-set:union set from to)))))
           tree)))
     
     (with-output-to-file "harro.dot"
       (lambda ()
         (let ((tree (minimum-spanning-tree (complete-graph (make-random-points 10)))))
           (pp (hash-table->alist tree) (current-error-port))
           ;; (debug (length (hash-table->alist tree))
           ;;        (hash-table-keys tree)
           ;;        (hash-table->alist tree))
           ;; (- (apply + (map length (hash-table->alist tree)))
           ;;    (length (hash-table->alist tree)))
