    ...
    (define (write-agent-as-dot points
      ...
      (let ((displayed (make-hash-table))
        ...
        (define (node-maybe-display state label)
          ;; (debug start goal state)
          ;; (let ((harro '********************************************))
          ;;   (debug state
          ;;          goal
          ;;          (when (equal? goal state)
          ;;            harro)))
             ...
             (let ((coordinate
               ...
               (format #t "~a [pos=\"~a,~a\"~a];"
                       ...
                       (cond ((equal? state start)
                              ", shape=circle, label=S, fontsize=64")
                             ((equal? state goal)
                              ", shape=circle, label=G, fontsize=64")
                             (else ""))))
             #t)))
