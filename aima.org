...
*** TODO Embedded R
    ...
    #+BEGIN_SRC c :tangle Reval.c
      ...
      #include <chicken.h>
        ...
        assert(strcmp(function, "cat") == 0);
      C_word Reval(char* name, C_word args) {
      
        C_word car = C_u_i_car(args);
        C_word cadr = C_u_i_cadr(args);
      
        assert(C_fixnump(car));
        assert(C_fixnump(cadr));
      
        int augend = C_num_to_int(car);
        int addend = C_num_to_int(cadr);
      
        assert(augend == 3);
        assert(addend == 4);
      
        SEXP fun = Rf_findFun(Rf_install(function), R_GlobalEnv);
        PROTECT (fun);
      
        SEXP e = allocVector(LANGSXP, nargs + 1);
      
        PROTECT(e);
      
        SEXP arg1 = NEW_INTEGER(1);
        INTEGER_DATA(arg1)[0] = augend;
      
        SEXP arg2 = NEW_INTEGER(1);
        INTEGER_DATA(arg2)[0] = addend;
        ...
        int error = 0;
        SEXP val = R_tryEval(e, R_GlobalEnv, &error);
        UNPROTECT(2);
      
        printf("%d", error);
        printf("%s", IS_LIST(value) ? "list" : "");
        printf("%s", IS_LOGICAL(value) ? "logical" : "");
        printf("%s", IS_INTEGER(value) ? "integer" : "");
        printf("%s", IS_NUMERIC(value) ? "numeric" : "");
        printf("%s", IS_CHARACTER(value) ? "character" : "");
        printf("%s", IS_VECTOR(value) ? "vector" : "");
        printf("%s", IS_RAW(value) ? "raw" : "");
        printf("%s", IS_COMPLEX(value) ? "complex" : "");
        printf("%s", IS_VECTOR(GET_CLASS(value)) ? "vectorclass" : "");
        printf("%d", GET_LENGTH(GET_CLASS(value)));
        printf("%d", GET_LENGTH(value));
        int evaluatum = INTEGER_DATA(value)[0];
        if (C_fitsinfixnump(evaluatum))
          C_return(C_fix(evaluatum));
      ...
      (define (Reval function . args)
        ((foreign-lambda*
          ...
          ((c-string function)
           ...
           (int nargs))
          #<<END
          C_return(Reval(function, args, nargs));
      END
      )
         function
         args
