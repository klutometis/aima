...
* TODO 6.17
  ...
  #+BEGIN_SRC scheme
    ...
    (let (;; (graph (alist->hash-table '((a b d)
          ;;                             (b a c)
          ;;                             (c b d)
          ;;                             (d c a))))
          (visited (make-hash-table)))
      (debug (sort (hash-table-keys graph) > (lambda (node) (length (hash-table-ref graph node)))))
      (let iter ((nodes (hash-table-keys graph)))
        (unless (null? nodes)
          (let visit ((parent (car nodes))
                      (path '()))
            ;; (debug parent
            ;;        path
            ;;        (hash-table-ref/default visited parent #f))
            (unless (hash-table-ref/default visited parent #f)
              (hash-table-set! visited parent #t)
              (for-each (lambda (child)
                          ;; (debug child (hash-table-ref/default visited child #f))
                          (when (and (hash-table-ref/default visited child #f)
                                     (not (null? path))
                                     (not (eq? child (car path))))
                            (debug 'child-cycle parent child path))
