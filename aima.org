    ...
    #+BEGIN_SRC scheme
      ...
      (let* ((world (make-preferential-depth-first-world 20))
        ...
        (let ((stateful-agent (make-stateful-graph-agent start))
              ...
              (steps 1000)
              (font-size 24))
          (let ((stateful-directory
                 (simulate-graph (copy-world world)
                                 stateful-agent
                                 steps
                                 width
                                 height
            ...
            (let ((composite-directory (create-temporary-directory)))
              (system* "cd ~a && for i in *; do echo $i; convert +append $i ~a/$i ~a/$i; done"
                       ...
                       random-directory
                       composite-directory)
              ((make-animation-finalizer composite-directory "composite-agent"))))))
      
    #+END_SRC
*** TODO Allow specification of random-seeds
