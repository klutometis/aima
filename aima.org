  ...
  #+BEGIN_SRC scheme :tangle 3.7.scm :shebang #!/usr/bin/env chicken-scheme
    ...
    (define (depth-first start end neighbors)
      (let ((visited (make-hash-table)))
        (let search ((frontier (list start))
                     (path '()))
          ;; (debug frontier)
          (if (null? frontier)
              (debug 'search-failed)
              (let* ((parent (car frontier))
                     (children (hash-table-ref neighbors parent)))
                ;; (debug parent)
                (hash-table-set! visited parent #f)
                (if (eq? parent end)
                    (cons parent path)
                    (let ((unvisited-children
                      ...
                      (search (append unvisited-children (cdr frontier))
                              (cons parent path)))))))))
    
    (define (predecessor-path predecessor end)
      (let iter ((path (list end)))
        (let ((predecessor
               (hash-table-ref/default predecessor (car path) #f)))
          (if predecessor
              ...
              (let* ((parent (queue-remove! frontier))
                ...
                (if (eq? parent end)
                    ...
                    (let ((unvisited-children
                      ...
                      (for-each
                          (lambda (child)
                            (queue-add! frontier child)
                            (hash-table-set! predecessor child parent))
                        unvisited-children)
                      (search))))))))
