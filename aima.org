...
** TODO b
   #+BEGIN_SRC scheme :tangle 4.3b.scm :shebang #!/usr/bin/env chicken-scheme
     ...
     (define (logarithmically-biased-random-real)
       (log (+ (* (- (exp 1) 1) (random-real)) 1)))
     
     (define logarithmic-bias (make-parameter 5))
     
     (define logarithmically-biased-random-real
       (case-lambda
        (() (logarithmically-biased-random-real (logarithmic-bias)))
        ((bias)
         (let ((r
                (/ 1 (log (+ (* (- (exp bias) (exp 1)) (random-real)) (exp 1))))))
           (debug r
                  (+ (* (- (exp bias) (exp 1)) (random-real)) (exp 1))
                  (log (+ (* (- (exp bias) (exp 1)) (random-real)) (exp 1)))
                  (/ 1 (log (+ (* (- (exp bias) (exp 1)) (random-real)) (exp 1)))))
           r))))
     
     (define (logarithmically-biased-random n)
       (inexact->exact (floor (* n (logarithmically-biased-random-real)))))
     
     (define (logarithmically-biased-random n)
       (inexact->exact (floor (log (+ (* (random-real) (- (exp n) 1)) 1)))))
     
     ;; (define (linearly-biased-random n)
     ;;   (inexact->exact (floor (log (+ (* (random-real) (- (exp n) 1)) 1)))))
     ...
     (define (genetic-algorithm population fitness reproduce mutate!)
       (let iter ((population population)
                  (i 1000))
         ;; (debug (map fitness population))
         (if (negative? i)
             (car (sort-population population fitness))
             (let ((new-population
                    (do ((new-population
                          '()
                          (cons
                           (let ((offspring
                                  (receive (x y)
                                    ...
                                    (reproduce x y))))
                             (if (< (random-real) 0.05) (mutate! offspring))
                             offspring)
                           new-population)))
                        ((= (length new-population)
                         ...
                         new-population))))
               (debug (abs
                       (- (fitness (car population))
                          (fitness (car new-population)))))
     ...
     (define (genetic-tsp points)
       (let ((population (populate-tour 100 points)))
     ...
     (let ((points (make-random-points 10)))
       (let ((genetic-tour (genetic-tsp points))
