...
** TODO b
   ...
   #+BEGIN_SRC scheme
     ...
     ;;;
     ;;; Can step be part of the world object? It's currently only a
     ;;; hash-table.
     (define (write-world-as-dot world agent step)
       (display "digraph G {")
       (display "node [style=filled, fontname=monospace];")
       (display "edge[fontname=monospace];")
       ;; Phew: viewports are specified in points at 72 per inch; size is
       ;; specified in pixels at 96 per inch.
       (let ((width 1600)
             (height 900))
         (let ((width-in-inches (/ width 96))
               (height-in-inches (/ height 96)))
           (format #t "graph [fontsize=48.0, viewport=\"~a,~a\", ratio=fill, size=\"~a,~a!\", label=\"Score: ~a; step: ~a\"];"
                   (* width-in-inches 72)
                   (* height-in-inches 72)
                   width-in-inches
                   height-in-inches
     ...
     (define (write-world-as-gif world agent)
       (receive (input output id)
         (process "neato -Tgif")
         (with-output-to-port output
           (lambda () (write-world-as-dot world agent)))
         ...
         (hash-table-walk
          ...
          (lambda (from to)       
            (let* ((rest (make-list (- 4 (length to)) no-passage))
              ...
              (let ((location
                     ;; TODO: An all-dirty world to really differentiate
                     ;; scores.
                     (make-location
                      ;; (if (< (random-real) 0.5)
                      ;;     clean
   ...
   #+END_SRC
*** DONE TODO: Figure out the correct aspect ratio for [[http://support.google.com/youtube/bin/static.pyhl%3Den&guide%3D1728585&topic%3D1728569&page%3Dguide.cs][youtube]].
