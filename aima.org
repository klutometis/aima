...
*** TODO Embedded R
    ...
    #+BEGIN_SRC c :tangle Reval.c
      ...
      C_word Reval(char* name, C_word args) {
        ...
        SEXP function = Rf_findFun(Rf_install(name), R_GlobalEnv);
        ...
        if (!error) {
          ...
          case REALSXP:
            printf("data: %f; ", REAL(Rvalue)[0]);
            value = C_alloc(C_SIZEOF_FLONUM);
            C_flonum(&value, REAL(Rvalue)[0]);
            break;
          case INTSXP:
            value = C_alloc(sizeof(C_word));
            *value = C_fix(INTEGER(Rvalue)[0]);
            break;
          }
          C_return(*value);
        }
    ...
    #+BEGIN_SRC scheme :tangle R-test.scm
      ...
      (define (Reval function . args)
        ...
        ((foreign-safe-lambda scheme-object "Reval" c-string scheme-object)
         function
         args))
      ...
      ;; (Reval "vector")
      ;;; INTSXP
      (debug (Reval "c" 2.023989823989823))
      ;; (debug (Reval "c" 1))
      ;; (debug (Reval "c" 2))
      ;; (debug (Reval "c" 3))
      ;;; STRSXP
