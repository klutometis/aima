...
*** Genetic algorithms
    ...
    - Intuitively, the advantage comes from the ability of crossover
      ...
      - [Each piece of information has to be independently useful, I
...
*** Partial observation
    ...
    - In the belief-state space, the problem is fully observable
      ...
      - [This is an interesting problem: human-like agents whose
    ...
    - Physical problem P defined by: Actions_P, Result_P, Goal-Test_P,
      ...
      - Actions :: If illegal actions have no effect on the
                   ...
                   states: \[Actions(b) = \Union_{s \in
                   b}Actions_P(s)\]
      - Transition model :: Deterministic: \[b\prime = Result(b, a) =
           \{s\prime : s\prime = Result_P(s, a) and s \in b\}.\]
           Nondeterministic: \[b\prime = Result(b, a) = \{s\prime:
           s\prime \in Results_P(s, a) and s \in b\} = \Union_{s\in
           b}Results_P(s, a)\]. Process of generating the new belief
           state after the action is called the prediction step.
