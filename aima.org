...
* TODO 6.1
  ...
  #+BEGIN_SRC scheme
    ...
    (define make-assignment make-hash-table)
    
    (define make-assignment
      (alist->hash-table `((wa . ,unassigned)
                           (nt . ,unassigned)
                           (q . ,unassigned)
                           (nsw . ,unassigned)
                           (v . ,unassigned)
    ...
    (define assignment-copy hash-table-copy)
    ...
    ;; (debug (complete? (alist->hash-table `((a . 2))))
    ;;        (complete? (alist->hash-table `((a . ,unassigned)))))
    ...
    (define (backtrack assignment csp)
      (if (complete? assignment)
          ...
          (let ((var (select-unassigned-variable csp)))
            (for-each
                (lambda (value)
                  (when (consistent? value assignment)
                    (assignment-add assignment var value)
                    (let ((inferences (inference csp var value)))
                      (unless (failure? inference)
                        (assignment-add-inferences assignment)
                     ...
                     (if (zero? (length (hash-table-ref (csp-domains csp) x)))
                         ...
                         (begin
                           (for-each (lambda (neighbor)
                             ...
                             (delete y (hash-table-ref (csp-graph csp) x)))
                           (iter)))
                     (iter))))))))
