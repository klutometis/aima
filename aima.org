...
* DONE 2.8
  ...
  #+BEGIN_SRC scheme
    ...
    ;;; It would be nice if we could do some update logic here and
    ;;; separate that from the performance measure.
    (define (make-performance-measuring-environment update-score
                                                    performance-measure)
      (update-score (performance-measure)))
    ...
    (define (make-reflex-vacuum-agent location)
      (make-vacuum-agent
       location
       0
    ...
    (define (make-vacuum-score-updater agent)
      (lambda (score)
        (vacuum-agent-score-set! agent (+ (vacuum-agent-score agent)
                                          score))))
    
    (let ((world (make-vacuum-world dirty clean))
          (agent (make-reflex-vacuum-agent right)))
