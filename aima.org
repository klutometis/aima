...
** DONE d
   ...
   #+BEGIN_SRC scheme :tangle tsp.scm
     ...
     (define graph-tour
       (case-lambda
        ((tour strategy output)
         (graph-tour tour strategy output #f #f))
        ((tour strategy output tree visited)
         (let ((data (tour->data tour))
           ...
           (call-with-values (lambda () (tour->vectors tour))
             (lambda (x y xend yend)
               ...
               (if tree
                   (call-with-values (lambda () (tree->vectors tree))
                     (lambda (x y xend yend)
                       (R (assign "x_tree" ,x))
                       (R (assign "y_tree" ,y))
                       (R (assign "xend_tree" ,xend))
                       (R (assign "yend_tree" ,yend))
                       (debug x y xend yend tree)
                       (R
                        (plot
                         (%+%
                          (%+%
                                ...
                                (%+%
                                 (%+%
                                  ...
                                  (geom_segment (aes x: x
                                                     ...
                                                     yend: yend
                                                     color: ,strategy)))
                                 (geom_text (aes x: (+ x (/ (- xend x) 2))
                                                 y: (+ y (/ (- yend y) 2)))
                                            label: (signif (sqrt (+ (^ (- xend x) 2)
                                                                    (^ (- yend y) 2)))
                                                           digits: 2)))
   ...
   #+BEGIN_SRC scheme :tangle 3.30.scm :shebang #!/usr/bin/env chicken-scheme
     ...
     (let* ((graph (complete-graph (make-random-points 10)))
       ...
       (let* ((tour (A*-tour start graph points))
         ...
         ;; (graph-tour tour "A*" "img/3-30.png")
         2))     
   #+END_SRC
