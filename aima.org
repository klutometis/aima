...
** TODO Plotting Voronoi
   ...
   #+BEGIN_SRC R :tangle voronoi.R :shebang #!/usr/bin/env Rscript
     ...
     ## for (key in keys(vertices)) {
     ##   vertex <- vertices[[key]]
     ##   cat(sprintf('(vertex %s %s %s)', key, vertex$x, vertex$y))
     ## }
     
     ## for (key in keys(neighbors)) {
     ##   cat(sprintf('(neighbor %s %s)',
     ##               key,
     ##               do.call(paste, as.list(neighbors[[key]]))))
     ## }
     ...
     ## debug(readLines("/tmp/fifo", n=1))
     ## q()
     
     path1 <- tiles[sample(nrow(tiles), 10), ]
     path2 <- tiles[sample(nrow(tiles), 10), ]
     path3 <- tiles[sample(nrow(tiles), 10), ]
     ...
     qplot(tiles$x1,
       ...
       ## also,
       ## <https://groups.google.com/forum/?fromgroups=#!topic/ggplot2/09CBpXV0YyU>
       ## scale_fill_manual(values=c("depth-first", "breadth-first", "bidirectional")) +
       scale_color_manual("harro",
                          values=c(1, 2, 3),
                          breaks=c('breadth-first',
                            'depth-first',
                            'bidirectional'),
                          labels=c('eins', 'zwei', 'drei')) +
