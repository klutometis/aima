...
* TODO 4.12
  ...
  #+BEGIN_SRC scheme
    ...
    (define (make-agent-random-walk start)
      (make-agent
       start
       0
       (lambda (points goal?)
    ...
    (define (write-dot-preamble width height)
     (display "digraph G {")
     ;; (display "node [shape=point, width=0.25];")
     (display "node [shape=point];")
     (let ((width-in-inches (/ width 96))
           (height-in-inches (/ height 96)))
       (format #t "graph [ratio=fill, viewport=\"~a,~a\", size=\"~a,~a!\"];"
               (* width-in-inches 72)
               (* height-in-inches 72)
               width-in-inches
               height-in-inches)))
    ...
    (define (write-agent-as-dot points
                                ...
                                previous-state
                                previous-action)
      (write-dot-preamble 1600 900)
      ;; (display "digraph G { graph [size=\"\"]; node[shape=point];")
      (let ((displayed (make-hash-table))
            (linear-scale (* 5 72)))
        (define (node-maybe-display node label)
          (hash-table-update!
           displayed
           node
           identity
           (lambda ()
             (let ((coordinate (hash-table-ref coordinates node)))
               (format #t "~a [pos=\"~a,~a\"];"
                       label
            ...
            (hash-table-walk whither->action
              (lambda (whither action)
                (let ((whence-label
                  ...
                  ;; (debug whence whither previous-state)
                  ;; (if (equal? whence previous-state)
                  ;;     (format #t "~a [width=0.5];" whither-label))
                  ;; (format #t "~a -> ~a;"
                  ;;         whence-label
                  ;;         whither-label)
    ...
    (define (make-agent-online-dfs start next-frame)
      (make-agent
       start
       0
       ;; Let's reset the coordinates after teleporting, somehow.
       (let ((coordinates (make-hash-table))
         ...
         (lambda (state goal?)
           ...
           ;;        (hash-table->alist untried)
           ;;        (hash-table->alist unbacktracked)
           ;;        previous-state
           ;;        previous-action)
           ;; (with-output-to-file (create-temporary-file)
           ;;   (lambda () (write-agent-as-dot result coordinates labels)))
           ...
           ;;        previous-state
           ;;        previous-action
           ;;        (hash-table-ref/default untried state #f)
           ;;        (hash-table-ref/default unbacktracked state #f))
           (if goal?
               (begin
               ...
               (begin
                 ...
                 (if previous-state
                   ;; (hash-table-set!
                   ;;  result
                   ;;  (cons previous-state previous-action)
                   ;;  state)
                     (begin
                       (hash-table-update!
                        result
                       ...
                       (unless (hash-table-exists? coordinates state)
                         (hash-table-set!
                          ...
                          (let ((previous-coordinates
                            ...
                            (make-point (+ (point-x previous-coordinates)
                                           (point-x previous-action))
                                        (+ (point-y previous-coordinates)
                     ...
                     (unless (hash-table-exists? coordinates state)
                       (hash-table-set!
                        ...
                        origin)))
                 ;; (debug state
                 ;;        (hash-table-ref/default untried state (make-stack))
                 ;;        (hash-table-ref/default unbacktracked state (make-stack)))
                 (if (stack-empty? (hash-table-ref/default untried state (make-stack)))
                     (if (stack-empty? (hash-table-ref/default unbacktracked state (make-stack)))
                         ...
                         (let* ((backtrack (stack-pop! (hash-table-ref unbacktracked state)))
                           ...
                           ;; (debug 'harro)
                           ;; (debug backtrack
                           ;;        (hash-table->alist (hash-table-ref result state))
                           ;;        state)
                           (set! previous-action (make-point (- (point-x backtrack-action))
                                                             (- (point-y backtrack-action))))
                           (set! backtrack? #t)
            ...
            (dotimes (step 500 agent)
              (let* ((agent-point (agent-point agent))
                ...
                (let* ((goal? (equal? agent-point goal))
                       (move ((agent-program agent) relative-points goal?)))
                  (debug move)
                         ...
                         (new-point
                          (if (and (zero-motion? move) goal?)
                              ...
                              (unless (stop? move)
                                ...
                                ;; (debug (hash-table->alist relative->visible-points))
                                (car (hash-table-ref relative->visible-points move))))))
                    ;; (debug agent-point move new-point goal?)
...
** TODO Need a generic animation facility.
** TODO What the hell: let's add some Bach or Liszt to the animations.
** DONE 3.22
