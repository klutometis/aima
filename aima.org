...
* TODO 6.17
  ...
  #+BEGIN_SRC scheme
              ...
              (let ((node (car nodes)))
                (if (non-trivial-cycle? graph visited node)
                    (begin
                      ...
                      (iter (cdr nodes) (cons node cutset)))
                    (iter (cdr nodes) cutset)))))))
    
    (define (subgraph graph nodes)
      (let ((subgraph (make-hash-table)))
        (for-each
            (lambda (node)
              (debug node (hash-table-keys graph))
              (hash-table-set!
               subgraph
               node
    ...
    (let ((graph (random-map 20)))
      (call-with-values (lambda () (cutset graph))
        (lambda (cutset complement)
          (let ((subgraph (subgraph graph cutset)))
            ...
            (let ((nodes (hash-table-keys graph)))
              ...
              (display-map-as-png
               complement
               (alist->hash-table
                (zip-alist (graph-nodes complement)
                           (make-list (length (graph-nodes complement)) 'blue)))))))))
